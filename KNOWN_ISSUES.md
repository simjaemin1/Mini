...
* 원인 자리 1 — `fireHandoff` 페이로드(`server/zone.js:9174~9198`)에 `toolItems` 가 **없다.**
  대신 `tools` 를 싣는데, 그건 welcome 스스로 `// 옛 호환 (사용 X)` 라고 적어 둔 죽은 필드다.
* 원인 자리 2 — 도착 경로(`server/zone.js:2762~2781`)가 `pending` 만 읽고 central 행을 **안 읽는다.**
...
**상태**: 미해결 · **근거**: 실서버 31분 연속 관측(`BENCHMARK.md` §3-B "남은 꼬리의 정체")
...
GC 는 아니다: 31분간 정지 1,520회 중 200ms 초과 4회, 최대 598ms(문제 틱의 1/56).
