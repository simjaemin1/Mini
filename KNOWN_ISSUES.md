...
### N.6 ★★존을 넘으면 **도구가 영구히 사라진다**
**상태**: 미해결(측정 배치라 **고치지 않음**) · **근거**: 로컬 2존 실측(`BENCHMARK.md` §4-C)
...
### N.8 `ZONE_TEST_INV` 가 central 이 살아 있으면 무력화된다
**상태**: 하네스 제약(운영 영향 없음) · 게스트도 central 행 복원 경로를 타면서
