...
### N.11 게스트가 **같은 틱에 재접속**하면 3번에 1번 걷던 자리를 잃는다 (T49 2026-09-02 · 하네스가 잡았다)

`test-guest-rejoin ②` 가 전수 러너에서 빨갰다. 손으로 18판을 더 돌려 **귀속을 갈랐다**:

```
  ① 한가한 기계 · 지금 하네스 ×6      빨강 2       ← 부하와 무관하다
  ② 바쁜 기계   · 지금 하네스 ×6      빨강 2
  ③ 바쁜 기계   · **옛 하네스** ×6    빨강 2       ← T10 수리 전 판도 똑같이 빨갛다
                                                    ⇒ 하네스가 만든 것이 아니다
```
...
```
  통과할 때   걷던 자리와 14~20px   · 낡은 저장 좌표와 233~244px      ← 몸을 물려받았다
  실패할 때   걷던 자리와 250~256px · 낡은 저장 좌표와 **0px**        ← 낡은 행에서 다시 태어났다
```

**어디인가.** `zone.js:3125~3145` 의 B-6 "살아 있는 몸 인수인계"는 새 소켓이 붙는 순간
**옛 세션이 아직 `players` 에 서 있을 때만** 돈다. 클라의 `ensurePrimaryConnection` 은
같은 틱에 close→connect 하므로 보통은 서 있다 — 그런데 close 가 먼저 처리되면
`ws.on('close')`(`zone.js:4341~`)가 `players.delete` 를 해 버려 **물려받을 몸이 없다.**
그러면 새 세션은 central 을 읽는데, close 가 같이 던진 `savePlayer`(fire-and-forget)와
그 읽기가 **또 경합**한다. 읽기가 이기면 낡은 좌표로 태어난다 — 실측 0px 이 그 자리다.

⇒ **두 개의 async 가 순서 보장 없이 같은 행을 읽고 쓴다.** 3번에 1번은 읽기가 이긴다.

**고치지 않았다** — T49 는 하네스 배치이고 게임 코드 diff 0 이 합격 조건이다. 회부한다(영역 Z).
설계 갈래 둘: ⓐ close 가 `savePlayer` 를 **기다린 뒤** `players.delete` 한다(순서를 만든다) ·
ⓑ 새 세션이 central 을 읽기 전에 **같은 신원의 최근 종료 몸**을 짧게 붙잡아 둔다(유예 창).
어느 쪽이든 "몸의 정본이 어디인가"(N.6 과 같은 물음)를 정하는 일이다.

★이 결함은 **오늘 생긴 게 아니다.** 옛 하네스도 같은 비율로 빨갛다. 전수 러너에서 여태
안 보인 이유는 3분의 1 확률이라 세 번의 전수에서 우연히 다 통과했기 때문이다
(0.67³ ≈ 0.30 — 충분히 있을 수 있는 일이다). ⇒ **깜빡이는 하네스를 "가끔 빨갛다"로 넘기면
그 밑의 제품 경합이 그만큼 안 보인다.**
