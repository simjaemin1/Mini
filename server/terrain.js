...
//   ⚠클라 거울 `public/terrain.js` 에 **같은 함수**가 있다(한쪽만 고치면 보이지 않는 물·바위가 생긴다).
...
function _getZoneTerrain(zoneId) {
  ...
  if (hc[zoneId]) {
    ...
    // 없으면 절차 ores 유지.
    // ★[T408] 경계 접합 — 이웃 존 피처 중 이 존 땅에 닿는 것(참조 · 사본 0).
    //   ⚠환경 손잡이를 **두지 않는다** — 클라 거울이 같은 합을 봐야 한다. 서버만 끄면 물·바위가
    //     서버와 화면에서 갈린다(보이지 않는 벽). 되돌림은 코드 되돌림이다.
    data.seam = _seamRefs(zoneId, hc, metas);
  }
...
function isWaterCellLocal(zoneId, localX, localY) {
  ...
  // ★[T408] 이웃 존의 물 — 질의점을 이웃 로컬로 옮겨 **그 객체에** 묻는다(사본 0)
...
function isRockCellLocal(zoneId, localX, localY) {
  const t = ZONE_TERRAIN[zoneId];
  if (!t) return false;
  const S = _seamOf(t);   // ★[T408] 이웃 존의 능선·고개·골짜기 — 우선순위는 그대로(골짜기·고개 > 물 > 바위)
...
// ★★[2026-08-02c 재민 최종 확정 — 캐논] **"npc는 자잘광산은 평생 못 봐"**
//   영구 확정이다. 시대(era)를 아무리 열어도, 지도를 아무리 고쳐도 이 술어는 자잘을 안 본다.
//   ⇒ `minor` 플래그를 떼는 행위(승격) 자체가 금지다. 자잘 광맥 승격 설계안(설계_NPC철기산업_지도.md)은
//     이 확정으로 **폐기**됐다. 새 세션이 "NPC 에게 철을 주려면 자잘을 승격하면 된다"는 결론에 도달하거든,
//     그건 이미 재민이 기각한 길이다.
//   ★★[2026-08-03c~d 재민 확정 — **다른 길도 닫혔다. 이제 길은 하나다**]
//     *"철기는, 플레이어가 마을 아무데나 세울 수 있는 시스템을 만들 거라, 자연해소돼"*
//     ⇒ 위 줄이 가리키던 "다른 길(주요 광맥 자체의 광종 설계)"도 **불필요해졌다.**
//     철은 **플레이어가 자잘 철 광맥(184개) 옆에 마을을 세워** 들여온다.
//     ★그런데 마을을 세워도 **그 마을의 NPC 광부는 여전히 자잘을 못 본다** — 이 술어가 그대로이기 때문이다.
//       그래서 채광 주체를 정해야 했고, 재민이 확정했다: **"플레이어 노동만"**(2026-08-03).
//       플레이어가 캐서 마을 곳간에 넣으면 econ 의 `_cons` 가 잡혀 flowT 수요가 서고,
//       NPC 대장장이가 그걸 쓰고 교역으로 퍼진다 — **새 장치 0. 이 캐논은 한 글자도 안 바뀐다.**
//     ⇒ 새 세션이 "플레이어 마을이 생겼으니 그 마을만은 자잘을 보게 하자"에 도달하거든,
//       **그것도 승격이다.** 마을 단위 예외조차 만들지 마라.
