...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // ★★[T363 2026-09-23 · 재민 결정 #48 ⓑ] **이름 존재를 아무에게나 알려주던 문을 닫는다.**
    //   T310 §4 가 회부한 그 문이다: 비밀 0 · 열쇠 0 으로 *"이 이름이 등록돼 있나"* 를 셀 수 있었다.
    //   ★**지우지는 못했다 — 부르는 쪽이 클라만이 아니었다.** 실측(§0-ⓐ):
    //     `zone.js:4191` 게스트 갈래가 *"게스트가 남의 등록 이름을 달고 들어오나"* 를 이 문으로 막는다.
    //     그 물음의 술어는 `findAccount`(= `player_id = ? OR name = ?` **AND 비밀번호 있음`)이고,
    //     **다른 어떤 문도 그 술어를 안 쓴다** — `GET /player/` 는 PK 조회라 **승계된 계정을 놓친다**
    //     (배치 14 ① 이 고친 바로 그 결함의 거울). 지우면 게스트가 승계 계정의 이름을 달 수 있다.
    //   ⇒ **문은 남기고 바깥만 닫는다.** 라우트 수는 62 그대로고, 갈래가 `공개` → `안문` 으로 간다.
    //     클라는 이 문을 더 부르지 않는다(로비는 제출 때 `/auth` 응답으로 안다 — `30-n-net.js`).
    //   ⚠**남은 구멍은 따로 있다**(보고 §4): `GET /player/<이름>` 이 없는 이름에 404, 있는 이름에 200 이라
    //     그 자체로 같은 답을 준다. 그건 투영 문의 **존재 여부**라 이 카드의 범위 밖이다 — 회부.
    if (req.url === '/check_username' && req.method === 'POST') {
      if (!isInternal(req)) return denyOutside(res);   // ★[T363] 안 문 — 부르는 쪽은 존뿐이다
