...
//   ★0.5가 "정보 0"인 게 중요하다. a=0.5면 판단이 진실과 무상관이라 아무것도 안 배운 것과 같다.
//
//   ★채널은 **대칭**이다 — says = (rand<acc) ? 진실 : 반대.
//     ⇒ TPR(광석을 광석이라 함) = TNR(맥석을 맥석이라 함) = acc · FP율 = FN율 = 1−acc.
//     즉 acc 는 혼동행렬 **네 칸을 다 포함한 정확도**다(한쪽 방향의 성능이 아니다).
//
//   ★[재민 지적 — 내 설계 오류 정정] 문구 문턱을 **사후확률**로 잡았던 것을 정확도로 되돌렸다.
//     사후확률 = p·a/(p·a+(1−p)(1−a)) 은 **p에 의존**하는데, p는 셀마다 다른 연속장이다
//     (광맥1 중심 0.372 · 가장자리 0.005). 그래서 "p=0.3 기준 사후"는 지도 어디에도 없는
//     대표값이고, 그걸 문턱에 박으면 **문구가 자리의 p를 누설**한다 — 감정 스킬이 아니라
//     p 표시기가 된다. 게다가 사후는 방향마다 다르다:
//       acc 0.872 · p=0.30 → "광석" 74% 맞고 "맥석" 94% 맞다 (같은 문구인데 신뢰도가 다르다)
//       acc 0.872 · p=0.05 → "광석" 26% 맞고 "맥석" 99% 맞다
//     문구가 나르는 건 **그 사람 눈의 성능(acc)** 이어야 하고, 사후는 플레이어가 자리를 보고
//     감안할 몫이다(그리고 선광 성적표가 경험적으로 알려준다).
...
function mineIdPhrase(acc, saysOre, mineralKo) {
  // ★문턱은 **정확도**다(사후확률 아님 — 위 주석 참조). 0.5가 정보 0이므로 그 위로 얼마나
  //   올라왔는지가 곧 눈의 성능이다.  xp 환산(τ=400): 0.60→95 · 0.78→389 · 0.90→879
  if (acc < 0.60) return null;                                   // 열에 여섯 — 아직 못 본다, 판단 자체를 안 함
