...
//     C ±70% · H1.60 : 2.21 / 2.22 / 2.22 일  ← 그때의 채택
//   되돌리기: `EV_PRICE_UP=0.40 EV_PRICE_DOWN=0.40 EV_HYST=1.35` 로 그 판이 정확히 재현된다.
//
// ★★★재정박 2 [재민 확정 2026-09-05 · T94] — **세계가 옮겨 갔으므로 문턱을 옮긴다.**
//   T63 은 "문턱은 범인이 아니다"로 끝났다(늘어난 것의 ⅔가 재고였고, 가격 문턱으로는 재고를 못 깎는다).
//   **T73(생곡을 식량으로 계상)이 그 판정을 뒤집었다.** 되돌림 스위치(`T73_RAWGRAIN=0`)로 두 세계를
//   나란히 재 보면(시드 1020 · 같은 계측기):
//     Δ(되돌림 → T73) = +1,166건 중 **가격 +814(70%)** · 재고 +382 · 일 −30
//     그 가격 +814 의 정체는 **쌀·밀 두 품목**이다 — 급등 rice +297·wheat +227 · 급락 rice +243·wheat +232
//     (품목 **종수는 안 늘었다**: 급등 40종→39종 · 급락 41종→42종. 같은 품목이 더 자주 뛴 것이다.)
//   ⇒ 이번엔 늘어난 축과 손잡이가 **같다**. T63 이 처방 A 를 물린 이유가 여기서는 성립하지 않는다.
//
//   실지도 51마을 3시드 800일 · 같은 틱 스트림 위 장부 여덟(카오스 잡음 0) · ㉯ 값 유형 밀도:
//     ±70(T63 채택) : 1.90 / 2.01 / 1.94 일  ← **셋 중 둘이 하한 밑**
//     ±80 · H1.60   : 1.98 / 2.11 / 2.04 일
//     ±85 · H1.60   : 2.03 / 2.16 / 2.09 일  ← 여유 0.03 — 얇다
//     ±90 · H1.60   : 2.09 / 2.22 / 2.16 일  ← **채택**(세 시드 전부 안 · 최소 여유 0.09)
//     ±95 · H1.60   : 2.17 / 2.30 / 2.24 일  ← 두 읽기 모두 안이지만 **급락 −29%**
//   비용(시드 1020 · ±70→±90): 급등 −7.5% · 급락 −20.4% · **재고 유형 0건 · 일 유형 0건 · 게시 의뢰 0건**
//   (`HYST` 를 1.60 에 고정했기 때문이다 — HYST 는 재고 래치 해제도 쓰는 **공유 손잡이**다 · test-events ㊷).
//   ⚠**`PRICE_DOWN` 의 남은 여행 거리는 0.10 뿐이다** — `dnOn = 1 − DOWN` 이라 DOWN ≥ 1.0 이면
//     급락이 **구조적으로 0** 이 된다(T63 이 잰 절벽). T86(식단 예산)이 밀도를 더 내리면
//     이 손잡이로는 못 넘는다 ⇒ 그때는 손잡이가 아니라 **읽기나 세계**를 봐야 한다(회부).
//   되돌리기: `EV_PRICE_UP=0.70 EV_PRICE_DOWN=0.70` → T63/T73 판을 **비트 동일** 재현.
...
const CFG = {
  ...
  PRICE_UP: _num('EV_PRICE_UP', 0.90),       // +90% 이탈 → 급등 ★채택값[T94] (T63 판은 0.70)
  PRICE_DOWN: _num('EV_PRICE_DOWN', 0.90),   // −90% 이탈 → 급락 ★채택값[T94] (T63 판은 0.70 · 1.0 이면 급락 0)
