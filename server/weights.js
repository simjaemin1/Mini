...
const DERIVED_KG = {
  // ★[빈손 시작 2026-08-28] 조잡한 석기 — **정품(tool 1.20 · weapon 1.50)보다 무겁다.**
  //   근거: 다듬지 않은 자갈 날을 짧은 나뭇가지에 동여맨 것이라 균형이 나쁘고 날 몫이 두껍다.
  //   (재료 합은 자갈 2×0.60 + 잔가지 0.40 + 풀 2×0.30 = 2.2kg 이지만, 깨 내고 다듬는 과정에서 절반쯤 떨어져 나간다.)
  crude_axe:    1.40,
  crude_pick:   1.50,  // 날 몫이 더 커 도끼보다 무겁다(자갈 3)
  crude_blade:  1.10,  // 자루가 짧다 — 셋 중 가장 가볍고 가장 빨리 닳는다
  ...
  // ★★[T316 2026-09-19 · 어부 둘째 — `_carry` 흡수] **볏단** = 수확 한 칸 분의 곡물 단.
  //   여태 생활층은 이것을 `npc._carry`(품목 없는 **칸 수**)로 들고 다녔다 — 품목이 아니라
  //   플레이어 낙하·무게가 못 보는 칸이었다. 품목으로 옮기면서 무게가 필요해졌다.
  //   ★유도(지어낸 수 0): 한 칸의 수확이 곳간에 넣는 곡물은 `economy-sim.T100_K` 단위이고
  //     코어 표의 `food` 가 0.70kg/단위다 ⇒ **T100_K × 0.70 = 5.341659330985916 × 0.70 = 3.739161…**
  //   ⚠이 수를 손으로 고치면 `test-fish-act` 가 빨개진다 — 하네스가 그 곱을 **다시 계산해 대조한다**.
  grain_sheaf:  3.739161531690141,
  water_bottle: 1.00,  // 박 물병 + 물 — 표 milk 1.03 과 같은 물 한 되 무게
  // ★★[자염 배치 2026-09-01] 짠물(함수) — **물병과 같은 무게여야 한다.**
  //   갯벌에서 뜨면 `water_bottle` 이 `brine` 으로 바뀌고 가마에서 도로 물병이 된다.
  //   무게가 다르면 **채수만으로 몸무게가 변한다**(병 안의 물이 짠물로 바뀐 것뿐인데).
  //   ⚠`salt.CFG.BRINE_KG` 와 같은 값이다 — 하네스가 두 수의 일치를 검사한다.
  brine:        1.00,
  // ★★[T12 지게 2026-09-01] **지게** — 통나무 2(6.0) + 풀 2(0.6) = 재료 합 6.6kg 인데 **3.00** 이다.
  //   근거는 이 표가 이미 쓰고 있는 것과 같다: **깎아 내면 줄어든다.**
  //   `pillar`(통나무 3 = 9.0 → 6.00) · `crude_axe`(재료 합 2.2 → 1.40) 와 같은 몫(대략 절반).
  //   지게는 **가지 두 개를 다듬어 A 자로 세우고 세장을 지른 것**이라 통나무 두 개 중 반 이상이 떨어져 나간다.
  //   ⚠이 3kg 은 **적재에 그대로 들어간다**(`carry.totalKg` 가 `equipment` 를 센다) —
  //     지게가 올려 주는 상한이 +8 이면 순이득은 +5 다. **공짜가 아닌 게 이 물건의 판단이다.**
  carrier:      3.00,
  ...
  item_drying_rack: 7.20, // ★[보존 배치 2026-08-31] 통나무 2(6.0) + 풀 4(1.2) — 재료 합 그대로(장대에 풀 끈)
  item_salt_kiln: 25.00,  // ★[자염 배치 2026-09-01] 석재 4(16.0) + 통나무 3(9.0) — 재료 합 그대로.
                          //   무겁다(가마다). 지고 해안까지 가는 것 자체가 이 배치의 첫 노동이다.
  // ★★[부패·보존 배치 2026-08-31 → **T17 ② 2026-09-02 이관**] 보존식 4종의 kg 은 여기 없다.
  //   그때는 *"표에 없어서 유도한 값"* 이라 이 표(플레이어 전용 유도값)가 정본이었다. T17 이
  //   보존식을 **econ 재화로 등재**하면서 `server/specialty.js` 가 그 자리를 가져갔고,
  //   위 `kgOf` 는 **specialty 를 먼저 본다** — 즉 여기 남겨 두면 그 순간부터 **읽히지 않는 사본**이다.
  //   (`test-weight ①` 이 그 사본을 잡는다: *"weights.js 안에 specialty 값을 옮겨 적은 줄이 없다"*.)
  //   ⇒ 값과 그 고증 근거는 `specialty.js` 의 네 줄로 **한 글자도 안 바뀌고** 옮겨 갔다:
  //     건어물 0.35 · 말린 과실 0.13 · 훈제육 0.62 · 절임 0.66.
};
// ★★[T54 갯벌 3차 2026-09-02] 민물 한 되와 갯벌 말린 것 둘 — **표의 주인이 채운다**(한 줄 주입).
//   왜 여기 숫자를 안 적나: 민물 한 되는 T3 이 세운 **한 되 1.00kg** 이고(병↔짠물↔민물이 서로 바뀐다),
//   마른 것 둘은 **원물 kg × 말리기 잔량비**라 둘 다 `tidal.js` 가 정본을 들고 있다.
//   여기 옮겨 적으면 잔량비를 고치는 날 이 줄만 뒤처진다(작물이 `kgOf` 에서 이미 쓰는 규약).
//   ⚠늦게 부른다 — `tidal` 은 `specialty` 를 무는데 이 파일도 그렇다(맞물림 금지 · `crops` 와 같은 자리).
{ let _T = null; try { _T = require('./tidal'); } catch (e) {} if (_T && _T.weightMap) Object.assign(DERIVED_KG, _T.weightMap()); }
