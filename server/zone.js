...
const DEBUG_COLLIDER = process.env.DEBUG_COLLIDER === '1' || process.env.ZONE_ID === 'korea'; // korea zone에서만 trace 켜기 — 다른 zone spam 방지
const BUILDING_COST = {
  wall:     { wood: 2, stone: 1 },
...
const VILLAGE_SAFE_RADIUS = 600; // 늑대 이 안에 spawn X (마을 안전구역)
{
  ...
  // === Phase 13.9.a — 디버그용 테스트 wall (korea zone 스폰 지점 (5120,5120) 주변) ===
  // 스폰 (5120,5120)에 서면 cell = (160, 160). 동쪽으로 가면 cell (161,160)으로 — E edge에 wall.
  // 남쪽으로 가면 cell (160, 161)로 — S edge (= cell (160,161)의 N edge).
  if (ZONE_ID === 'korea') {
    ...
    function addTestWall(cellCx, cellCy, side) {
      const wx = cellCx * BUILDING_SIZE;
      const wy = cellCy * BUILDING_SIZE;
      const data = { side, floor: 0 };
      const dbId = db.insertBuilding({ type: 'wall', owner_id: TEST_OWNER, owner_name: 'DEBUG', x: wx, y: wy, data: JSON.stringify(data) });
      const id = `b${nextBid++}`;
      const building = { id, dbId, type: 'wall', ownerId: TEST_OWNER, ownerName: 'DEBUG', x: wx, y: wy, data, floor: 0 };
      buildings.set(id, building);
      chunkManager.insertBuilding(building);
      console.log(`[${ZONE_ID}] 🧱 테스트 wall: cell (${cellCx},${cellCy}) side=${side} world=(${wx},${wy})`);
    }
    // 스폰 cell (160,160) 자체. 동·북·남·서 4면 다 — 사용자가 어느 방향으로 가도 막혀야.
    addTestWall(160, 160, 'E');      // 동쪽 (x=5152 라인) — 오른쪽으로 막힘
    addTestWall(160, 160, 'N');      // 북쪽 (y=5120 라인) — 위로 막힘
    addTestWall(160, 161, 'N');      // 남쪽 (cell 160,161의 N = cell 160,160의 S, y=5152 라인) — 아래로 막힘
    addTestWall(159, 160, 'E');      // 서쪽 (cell 159,160의 E = cell 160,160의 W, x=5120 라인) — 왼쪽으로 막힘
    console.log(`[${ZONE_ID}] 🧱 스폰 지점 (5120,5120) 4면 테스트 wall 배치 — 사용자는 cell 안에 갇힘`);
  }
