...
function computeNpcPath(npc, now) {
  ...
  //   (목표 셀 중심이 물·바위·도랑인 호출 45~53 % = 시간의 48~54 % · 강 **건너**(다른 뭍 덩어리)는 0 건 — 강 **속**이다).
  //   그 목표의 89 %는 생활 층이 넘긴 결정(npcLifeTick 거짓)의 ⑤ 배회가 `npcWorkX ± 80` 에서 뽑은 것이고,
  //   주민의 10.8 %는 태어날 때 받은 그 일터(villages.js 회관 둘레 180~320px 도넛 · 지형 안 봄)가 물·바위다 — 뿌리는 거기다(회부).
  //   ⇒ **목표 셀이 통행 불가면 A\* 를 아예 안 부른다.** `routePath` 가 첫 줄에 이미 가진 규칙
  //     (`if (blocked(sx,sy) || blocked(gx,gy)) return null;`)을 **걸음 쪽 부르는 자리에 같게** 댄 것이다(새 규칙 0).
  //   ★술어는 바로 아래 A* 에 `isWaterFn` 으로 주입하는 **그 함수**다(정본 하나 · 다리 칸은 이미 빠진다).
  //     셀 중심 식은 `findPath` 의 `cpx`(c*32+16) 그대로 — 새 수 0.
  //   ★같은 셀은 뺀다 — `findPath` 가 탐색 없이 답을 주는 자리다. `d < 48` 이 앞에서 이미 돌려보내므로
  //     여기 오지 않지만(셀 대각선 45.25px) 기대지 않고 글자로 뺀다.
  //   ⚠**비트 동일이 아니다**(처음엔 그렇게 믿었다가 `test-move-soa ⑩-b` 에 걸렸다): `localPath` 는 끝점이
  //     막혔는지 **안 본다**. 커널이 왕복 대칭을 위해 끝점을 **사전순으로 뒤집으면**(`_searchBegin` 의 `rev`)
  //     막힌 목표가 **출발**이 되고, 출발 칸은 아무도 검사하지 않으므로 **물에서 끝나는 길**이 나온다
  //     (실측: 찾은 길의 17.6 %). 켜면 그 길이 `null`(→ beeline)이 된다 — **바뀌는 것은 그 자리 하나뿐**이다
  //     (⑩-d: 열린 목표는 전수 비트 동일). 정본 커널을 고치는 것은 이 카드 밖이다(`path-core.js` 무접촉 · 회부).
  //   `npc._lastAStarAt` 은 위에서 이미 찍혔다 — 쿨다운 박자는 끔과 같다.
  if (T381_PATH_DEAD) {
