...
const VILLAGE_SAFE_RADIUS = 600; // 늑대 이 안에 spawn X (마을 안전구역)
{
  ...
  if (ZONE_ID === 'korea') {
    ...
    // 스폰 cell (160,160)에서 옆으로 떨어진 cell (163, 163) 4면 — 사용자가 박스 밖에서 시작.
    // 사용자가 spawn에서 동남으로 100px쯤 가야 박스 도달. 명확한 시각 검증 가능.
    addTestWall(163, 163, 'E');      // 동쪽
    addTestWall(163, 163, 'N');      // 북쪽
    addTestWall(163, 164, 'N');      // 남쪽 (cell 163,164의 N = cell 163,163의 S)
    addTestWall(162, 163, 'E');      // 서쪽 (cell 162,163의 E = cell 163,163의 W)
    console.log(`[${ZONE_ID}] 🧱 테스트 wall 박스 @ cell (163,163) 4면 — spawn (5120,5120)에서 동남 100px 거리`);
  }
