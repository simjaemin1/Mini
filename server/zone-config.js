...
const ZONES_BASE = {
  ...
  jungwon_n: {
    ...
    // ★★[T409 2026-09-26] 중원북 다리 — `plan-bridges-v2.js jungwon_n` 이 낸 그대로(T360 착지 조건 · 축 4방 · 폭 2셀 · 새 수 0).
    //   T373: 후보 25 → 시딩 11 · 55쌍 중 **불능 45**. 스폰 덩어리(3,035,798셀)와 서쪽·북쪽 큰 덩어리 셋이 강에 갈려 있었다.
    //   계획기를 수렴까지 돌려 도하 다섯을 얻었고, **하나씩 빼 보고**(빼도 도달이 같으면 버림) 셋만 남겼다:
    //     (16,1564)→(23,1564) 6칸 16셀 · (396,161)→(372,161) 23칸 50셀 · (538,2276)→(538,2264) 11칸 26셀 = **92셀**.
    //   버린 넷(16+16+18+26셀)은 존 가장자리 조각(수십 셀짜리 디딤돌)에 닿아 도달을 하나도 안 늘렸다.
    //   실셀 도달: 다리 끔 3,056,872셀(24.1%) → 켬 11,742,228셀(92.5%) · 시딩 11곳 **55/55쌍**(`test-jungwon-boot` ⓙ).
    //   ⚠**단광산**(후보 · 정본 좌표 (1431,103509))은 준산맥+강에 갇힌 17,032셀 주머니다 — 물 141칸(직선 도하 없음) · 바위 42칸.
    //     식량 하한으로 시딩되지 않아 쌍에 안 든다. 연다면 다리가 아니라 **고개**다(재민 · 값 안 짓는다).
    bridges: [16,1564,16,1565,17,1564,17,1565,18,1564,18,1565,19,1564,19,1565,20,1564,20,1565,21,1564,21,1565,22,1564,22,1565,23,1564,23,1565,396,161,396,162,395,161,395,162,394,161,394,162,393,161,393,162,392,161,392,162,391,161,391,162,390,161,390,162,389,161,389,162,388,161,388,162,387,161,387,162,386,161,386,162,385,161,385,162,384,161,384,162,383,161,383,162,382,161,382,162,381,161,381,162,380,161,380,162,379,161,379,162,378,161,378,162,377,161,377,162,376,161,376,162,375,161,375,162,374,161,374,162,373,161,373,162,372,161,372,162,538,2276,539,2276,538,2275,539,2275,538,2274,539,2274,538,2273,539,2273,538,2272,539,2272,538,2271,539,2271,538,2270,539,2270,538,2269,539,2269,538,2268,539,2268,538,2267,539,2267,538,2266,539,2266,538,2265,539,2265,538,2264,539,2264],
  },
  jungwon_s: {
  ...
  hanbando: {
    ...
    //   ★[T373 2026-09-23 보강] 실측으로 수가 붙었다(T360 §0-ⓒ): 어촌6 자리는 **20셀짜리 조각**이다.
    //     "뭍이 없다"가 아니라 **"설 자리가 20칸뿐"**이고, 그래서 다리로 풀 일이 아니다 —
    //     **섬이 아니라 설 자리가 없는 곳**이다(계획기 집계에서도 섬에서 뺐다 · T373 ①).
    ...
    //   ⚠**[T373 2026-09-23 정정] 이 줄은 T2(07-28) 그날의 판정이고 지금은 사실이 아니다.**
    //     아래 11차 재배치·물길 정합에서 다리를 더 놓는 사이에 광산2 는 **본토에 붙었다**.
    //     실측(T360 §0-ⓒ · 실셀 BFS · 서버가 보는 물 두 층): 광산2(셀 2007,218)는 **본토 #1
    //     7,298,105셀 안**이고 최단 폭 **0칸**이다. 후보 51곳 중 **50곳이 본토 한 덩어리**고,
    //     따로 떨어진 것은 **어촌6 하나**(20셀 조각)뿐이다. ⇒ 한반도엔 배가 필요한 자리가 **없다**.
    //     값은 한 칸도 안 바꿨다 — 낡은 판정을 지우지 않고 **정정만** 붙인다(왜 그렇게 적혔는지가 이력이다).
  ...
  nippon: {
    ...
    //   규격도 같다: 폭 2셀(교행) · k=0..len+1(양끝 뭍 접지) · flat [cx,cy,…].
    //   T343 이 닛폰에 마을을 세웠는데 교역 쌍의 2/3 가 서로 못 갔다. 실셀 BFS 로 갈라 보니
    //   섬 하나가 아니라 **덩어리 여럿**이었다(보고/T348 §0-ⓐ).
    //   ⚠★**처음 놓은 다리는 헛것이었다** — 계획기가 서버와 **다른 물**을 보고 있었다.
    //     서버의 통행 정본(`zone.js:818`)은 물을 둘 본다: 해안선 띠(`WATER_TILES`)와 손그림 강·호수.
    //     계획기는 **띠를 안 봤다.** 한반도에선 띠가 존의 4.4%(폭 2188셀)라 가장자리에만 있어 티가
    //     안 났지만, 닛폰은 폭 1562셀뿐이라 띠가 **16.0%** 로 안쪽까지 든다. 그래서 계획기가
    //     "본토"라 부른 자리가 서버에선 바다였고, 도하 5셀짜리 다리 14셀을 놓아도 **도달 쌍이 안 늘었다**.
    //     계획기가 같은 물을 보게 고친 뒤 다시 뽑은 것이 아래다(도하 **64셀** · 다리 **132셀**).
    //   ⇒ 이 다리가 잇는 것: 쿠라다(558,103셀) ↔ 본토(2,060,422셀 — 마트노·이즈이·스폰 도가이 광장).
    //     실측 도달 7쌍 → **9쌍**(21쌍 중) · 코스 격자도 같은 답 · 서버 로그 `다리구제 32`.
    //   ★**놓지 않은 것 다섯** — 계획기가 "200셀 내 본토 도하 없음 = 대양 분리"라 답한 덩어리들이다.
    //     가장 큰 것(1,529,254셀 · 네코로·노라마·지옹로·타카쿠)이 그 안에 있다.
    //   ⚠**[T360 2026-09-23 정정] 그것도 바다가 아니었다.** 계획기는 **본토 직행 광선**만 봤는데,
    //     그 덩어리는 사람이 안 사는 뭍(디딤돌)을 거치면 16+5+11칸이면 닿는다. 착지 조건에서
