...
const ZONES_BASE = {
  ...
  hanbando: {
    ...
    // ★★[배치 16 · 재민 확정 "해봐"] **마을 50개 세계.** 후보 51곳을 품질 게이트 없이 전부 심는다.
    //   근거는 배치 15 실측이다(보고_50마을_처방전.md): 51곳 전수 시딩 800일 3시드에
    //   생존 47·좀비 2·소멸 2(전부 1/3 시드)이고, 1,600일엔 소멸·좀비 0(총인구 9,100).
    //   32곳이 빠져 있던 이유는 땅이 아니라 선별 게이트였다 — 식량 하한 2.0(14곳)과
    //   마을 간격 12,000px(18곳). 상한은 병목이 아니었다(20→51 로 올려도 19곳 그대로).
    //   ★후보 51 → 실제 시딩 **50**: 어촌6 은 중심 주변 24셀에 뭍이 없어 시딩 단계에서
    //     물리적으로 스킵된다(품질 게이트가 아니라 "물 위엔 못 세운다"). 재민이 말한 50개와 일치.
    seedAllVillages: true,
    villageMax: 51,             // 전수 시딩이라 실질 무의미하지만 명시(모듈 VILLAGE_MAX·플레이어 마을 상한과 분리)
    ...
    // ★★[배치 16 ③] 스폰 — **농촌22 광장**. 50곳 배치를 보고 골랐다(배치 15 의 이월 항목).
    //   옛 스폰(벌말 광장, 셀 1093,2031)은 가장 가까운 마을이 212셀(6,780px) 밖이었다 —
    //   청크 활성 반경(1,200px)의 5.6배라 **첫 화면이 빈 들판**이었고, 그게 라이브 `bld=5` 의 정체다.
    //   왜 농촌22 인가(50곳 세계 실측 — 3시드 800일):
    //     · **중심성 1위** — 다른 49곳까지 평균 1,281셀(세계 중심 셀 1094,2032 바로 옆)
    //     · **가장 튼튼한 축** — 3시드 최소 인구 101 · 평균 115(소멸·좀비 이력 0)
    //     · 광장(plaza) 셀 위 · 통행 차단 건물 0 · 사유지 0 · 물 아님(실측)
    //   자리는 큰집(9×9, 중심 ±4셀) **남쪽 7셀**의 마당 셀 — 클라 시야 650px(≈20셀) 안에
    //   큰집(북 7셀)과 움집 2채(남 9셀)가 함께 들어온다.
    //   base × WORLD_SCALE(10) = (30848,59872) → 스폰 스냅(셀 중심) = (30864,59888) = 셀(964,1871).
    mainSquare: { x: 3084.8, y: 5987.2, name: '농촌22 광장' },
