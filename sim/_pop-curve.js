...
if (CHILD_SEED != null) {
  ...
  try {
    ...
    p.curve=global.__curve;p.aud=global.__aud;if(process.env.RESDUMP){const agg={},pr={};for(const v of r.VILS){for(const[k,q]of Object.entries(v.econ.storage||{}))agg[k]=(agg[k]||0)+q;const P=v.econ.prices||v.econ._prices||(v.econ.market&&v.econ.market.prices);if(P)for(const[k,q]of Object.entries(P))pr[k]=q;}p.resdump=Object.entries(agg).map(([k,q])=>k+':'+q.toFixed(0)+(pr[k]!==undefined?'@'+(+pr[k]).toFixed(2):'@?')).sort().join(' ');}p.seedRow = `시드${CHILD_SEED} 고기${(p.meat||0).toFixed(0)} 사냥꾼${p.hunterN||0} 인구${p.pop} 마을${r.VILS.length}/${r.initVil} 동기${r.bad} 청동도구${r.VILS.reduce((a, v) => a + (v.econ.storage.bronze_tool || 0), 0).toFixed(0)} 집${p.houses} 약재${(p.herb||0).toFixed(0)}/소비${(p.herbUsed||0).toFixed(0)} 채집꾼${p.foragerN||0} 뼈${(p.bone||0).toFixed(0)}/투입${(p.boneUsed||0).toFixed(0)} 활Q${(p.bowQmin==null?1:p.bowQmin).toFixed(2)}~${(p.bowQmax||1).toFixed(2)}(무기장${p.wsmithN||0}) 호피${(p.tigerhide||0).toFixed(1)}/교역${(p.thTrade||0).toFixed(1)}`;
