...
if (CHILD_SEED != null) {
  ...
  global.eval(PC.replace(/^function pickCenter/, 'global.pickCenter=function') + '\n' + BFS.replace(/^function bfsPath/, 'global.bfsPath=function') + '\n' + SP.replace(/^function setPath/, 'global.setPath=function') + '\n' + TP + '\n' + LIFE +
    "\nglobal.run=function(seed){TR=buildTerrain(seed);document.getElementById('seed').value=String(seed);console.log=function(){};lifeInit();lifeGM=L_START*1440;lifeLast=0;global.nowMs=0;lifeOn=true;global.rafCb=lifeLoop;var bad=0,initVil=VILS.length;global.__curve=[];for(var fr=0;fr<1500;fr++){global.frame();if(fr%10===0){var P9=0,B9=0;if(!global.__aud)global.__aud={slot:0,prod:0,fuel:0,gateH:0,gateK:0,vd:0,happy:0,health:0,prest:0,hunger:0,logi:0,hapV:0,heaV:0,dNeg:0};var A9=global.__aud;for(var k9=0;k9<VILS.length;k9++){var v9=VILS[k9].econ;P9+=v9.npcs.length;B9+=(v9._mapBeds||0); if(v9._kDbg){A9.vd++;var kd=v9._kDbg;var mn=Math.min(kd.slot,kd.prod,kd.fuel);if(mn===kd.fuel)A9.fuel++;else if(mn===kd.prod)A9.prod++;else A9.slot++;} var dd=v9._dpDebug;if(dd){var hc=Math.min(v9.housing!==undefined?v9.housing:1e9,v9._mapBeds!==undefined?v9._mapBeds:1e9);if(dd.gated)A9.gateH++;else if(v9.npcs.length>=dd.K*0.95)A9.gateK++;A9.happy+=Math.abs(dd.happy||0);A9.health+=Math.abs(dd.health||0);A9.prest+=(dd.prestige||0);A9.hunger+=Math.abs(dd.hunger||0);A9.logi+=Math.abs(dd.logi||0);if((dd.dP||0)<0)A9.dNeg++;} if(v9.lastStats){A9.hapV+=v9.lastStats.happiness||0;A9.heaV+=v9.lastStats.health||0;}} {let W9=0;for(var k8=0;k8<VILS.length;k8++)W9+=(VILS[k8].econ.storage.wood||0);global.__curve.push([Math.round(lifeGM/1440-L_START),P9,B9,Math.round(W9)]);}}if(fr%300===0)for(var k=0;k<VILS.length;k++)if(VILS[k].agents.length!==VILS[k].econ.npcs.length)bad++;}console.log=global.__log;return {bad:bad,initVil:initVil,VILS:VILS,world:ECON_WORLD};};");
  ...
  try {
    ...
    p.curve=global.__curve;p.aud=global.__aud;if(r.world&&r.world._distMatrixStats)p.distMx=r.world._distMatrixStats;/*★BFS 거리행렬 주입 검증(랩 lifeInit이 채움): 쌍수·유클리드 대비 증가율·도달불능*/if(r.world&&r.world._tradeAudit){const T=r.world._tradeAudit,rs=T.rs.slice().sort((a,b)=>a-b),q=f=>rs[Math.floor(f*(rs.length-1))]||0;p.tradeAud={n:T.n,bail:T.bail,reroute:T.reroute,p10:q(0.1),p50:q(0.5),p90:q(0.9),mean:+(rs.reduce((a,b)=>a+b,0)/(rs.length||1)).toFixed(3)};}if(process.env.JOBDUMP){p.jobdump=r.VILS.map((v,i)=>{const c=v.econ.counts||{};const vc={};for(const a of (v.agents||[]))vc[a.job||'?']=(vc[a.job||'?']||0)+1;const t=Object.entries(c).filter(([k,q])=>q>0).sort((a,b)=>b[1]-a[1]).slice(0,4).map(([k,q])=>k+':'+q).join(',');const t2=Object.entries(vc).sort((a,b)=>b[1]-a[1]).slice(0,4).map(([k,q])=>k+':'+q).join(',');return 'V'+i+' econ['+t+'] 시각['+t2+']';}).join(' | ');}if(process.env.RESDUMP){p.defo=r.VILS.map(v=>((v.econ._defoWood||0)).toFixed(1)).join(',')+' 유실:'+r.VILS.map(v=>((v._defoLost||0)).toFixed(1)).join(',');const agg={},pr={},eat={},dpb={};let cookN=0;for(const v of r.VILS){for(const[k,q]of Object.entries(v.econ.storage||{}))agg[k]=(agg[k]||0)+q;for(const[k,q]of Object.entries(v.econ._foodEaten||{}))eat[k]=(eat[k]||0)+q;for(const[k,q]of Object.entries(v.econ.dailyProductionBuf||{}))if(q>0.0005)dpb[k]=(dpb[k]||0)+q;cookN+=Math.round((v.econ.counts&&v.econ.counts.cook)||0);const P=v.econ.prices||v.econ._prices||(v.econ.market&&v.econ.market.prices);if(P)for(const[k,q]of Object.entries(P))pr[k]=q;}p.resdump=Object.entries(agg).map(([k,q])=>k+':'+q.toFixed(0)+(pr[k]!==undefined?'@'+(+pr[k]).toFixed(2):'@?')).sort().join(' ');p.eatdump=Object.entries(eat).filter(([k,q])=>q>0.001).map(([k,q])=>k+':'+q.toFixed(1)).sort().join(' ');p.proddump=Object.entries(dpb).map(([k,q])=>k+':'+q.toFixed(1)).sort().join(' ');p.cookN=cookN;}/*★유령 감사: 재고(resdump)+최종일 식단(_foodEaten)·생산(dailyProductionBuf) 흐름 — 반유령(축적) vs 정상(유계 순환) 런타임 판별. env 게이트라 회귀 무영향*/p.seedRow = `시드${CHILD_SEED} 고기${(p.meat||0).toFixed(0)} 사냥꾼${p.hunterN||0} 인구${p.pop} 마을${r.VILS.length}/${r.initVil} 동기${r.bad} 청동도구${r.VILS.reduce((a, v) => a + (v.econ.storage.bronze_tool || 0), 0).toFixed(0)} 집${p.houses} 약재${(p.herb||0).toFixed(0)}/소비${(p.herbUsed||0).toFixed(0)} 채집꾼${p.foragerN||0} 뼈${(p.bone||0).toFixed(0)}/투입${(p.boneUsed||0).toFixed(0)} 활Q${(p.bowQmin==null?1:p.bowQmin).toFixed(2)}~${(p.bowQmax||1).toFixed(2)}(무기장${p.wsmithN||0}) 호피${(p.tigerhide||0).toFixed(1)}/교역${(p.thTrade||0).toFixed(1)}`;
