...
P=[("const t=MOB_PREY[(Math.random()*MOB_PREY.length)|0],D=MOB_DEF[t];","const t=HUNT_SPECIES,D=MOB_DEF[t];/*랩 패치*/"),
 ...
 ("nearIn(mgrid,a.px,a.py,m=>{if(m.hp<=0||m.type==='🐯')return;/*★전담 부상 표적이 살아있으면 한눈 안 팖 · 마을은 호랑이 표적 금지*/",
  "nearIn(mgrid,a.px,a.py,m=>{if(m.hp<=0)return;/*랩: 호랑이 사냥 허용·전담 있으면 한눈 안 팖*/"),
