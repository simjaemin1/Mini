...
// 소스: server/era.js + server/specialty.js + server/hut-stages.js + server/stone-uses.js + server/village-layout.js + sim/economy-sim.js + sim/economy-sim-v2.js
;(function(root){
  var modules={};
  ...
  function req(p){ if(/hut-stages/.test(p))return modules.hut; if(/stone-uses/.test(p))return modules.stoneu; if(/village-layout/.test(p))return modules.vlayout; if(/era/.test(p))return modules.era; if(/specialty/.test(p))return modules.specialty; if(/economy-sim-v2/.test(p))return modules.v2; if(/economy-sim/.test(p))return modules.v1; return {}; }
