...
// ★★★[T367 2026-09-23 · 재민 확정 #12] **기본 0.9.** 재민 09-23: *"물리 파라미터 배수는 맞다."*
//   ⚠**새 수 0** — 0.9 는 T195 가 자리를 만들고 **T191/T200 이 잰** 값이다(T252 가 9건 중 가장 단단하다고 냈다:
//     +3.8% · 폭 3.0). 이 줄은 채택값을 1 에서 그 값으로 옮길 뿐, 감가율 두 수(0.02·0.01)는 그대로다.
//   되돌림은 **명시 `1`**(`T195_TOOL_WEAR=1`) — 그때 서버는 문을 **아예 안 연다**(`!== 1` 가드 · `server/villages.js`)
//   ⇒ 넷째 판과 **비트 동일**이다(보고/T367 §1).
const TOOL_WEAR_MUL_DEFAULT = 0.9;   // ★[T367] 채택값 — T191/T200 실측(새 수 0)
const TOOL_WEAR_MUL = (() => {
  const v = parseFloat((typeof process !== 'undefined' && process.env && process.env.T195_TOOL_WEAR) || '');
  return (Number.isFinite(v) && v > 0) ? v : TOOL_WEAR_MUL_DEFAULT;   // 미설정 = 0.9(기본 · T367)
})();
...
// ★★★[T367 2026-09-23 · 재민 확정 #11] **기본 켬.** 미설정이면 4판(밭 수확 순간 곳간)으로 돈다.
//   재민 09-23: *"규약은 배수 금지가 아니라 추상 없애기다 · 농부가 직접 밭 갈고 수확하는 순간
//   그 작물이 플러스되기로 했잖아."* — `addProduce(farmer)` 라는 **추상**을 걷어내는 것이 이 손잡이다.
//   되돌림은 **명시 `0`**(`T100_FIELD_YIELD=0`) — 그때 이 파일은 넷째 판과 **비트 동일**이다(보고/T367 §1).
//   ⚠값은 한 개도 안 지었다(새 수 0) — 4판 유도값 `T100_K`·`T100_ANCHOR_N` 은 09-05 것 그대로다.
const T100_FIELD_YIELD = process.env.T100_FIELD_YIELD !== '0';   // 미설정 = 켬(기본 · T367) · 되돌림 명시 '0'
...
function forageToGranary(v, item, units) {
  ...
  return actToGranary(v, item, units, '_t347InflowToday', '_t347PickN');
}
...
// ★★★[T367 2026-09-23 · 재민 확정 #11] **기본 켬.** 4판과 **한 처방**이다 —
//   밭 유입을 장부(`dailyProductionBuf`)가 못 보면 `surplusEMA.food` 가 무너져 마을의 3분의 2가
//   쌀을 쌓아 두고 자기를 적자로 읽는다(T186 §0-ⓑ'). 4판을 켜면서 이걸 끄면 그 세계를 켜는 셈이다.
//   되돌림은 **명시 `0`**(`T193_LEDGER=0`).
const T193_LEDGER = process.env.T193_LEDGER !== '0';   // 미설정 = 켬(기본 · T367) · 되돌림 명시 '0'
