...
// ★★★[T165 2026-09-10 · 이식] **계수가 엔진 정본이 됐다.**
//   T157 은 랩만 켜려고 계수를 밖(`world.happyWorkW`)에서 받았다. PM 판정(09-10): 이식하되
//   **상수 하나 + env 손잡이**로 두고 **기본은 끔**(비트 동일)이다. 랩은 번들로 따라온다.
//   ★새 수는 이것 하나이고 출처가 있다:
//     Oswald · Proto · Sgroi (2015) "Happiness and Productivity", *Journal of Labor Economics* 33(4).
//     무작위 처치(코미디 영상·간식)로 행복이 오른 집단의 생산성이 **약 12%** 높았다(초록 · 본문 10~12%).
//     처치는 *중립 → 행복* 이라 우리 축의 **0.5 → 1.0**(반쪽 폭)에 대응한다 ⇒ 0.5 × H = 0.12 ⇒ **H = 0.24**.
//   ⚠상한도 새 수가 아니다 — `H` 자신(`1 ± H/2`)으로 자른다(아래 `happyWorkMul`).
const T157_HAPPYWORK_H = 0.24;
const T157_HAPPYWORK = process.env.T157_HAPPYWORK === '1';   // 되돌림: 기본 **끔** = T157 이전 세계 비트 동일
// 이 마을에 걸릴 계수. 정본은 위 상수이고, `world.happyWorkW` 는 **A/B·랩 토글의 덮어쓰기**다
//   (숫자를 주는 자리가 아니라, 이미 있는 수를 잠깐 다른 값으로 재는 자리 — 랩은 0 과 정본만 오간다).
...
//   ⚠계수 `H` 는 위 `T157_HAPPYWORK_H` 하나다(T165 이식) — 부르는 쪽은 그걸 `happyWorkWOf(v)` 로 받는다.
...
//
//   ⚠**손잡이가 꺼져 있으면 한 글자도 안 바뀐다**(T165 기본) — `happyWorkWOf(v)` 가 0 을 돌려준다.
//     지금 그것을 주는 것은 **랩**(`lab/전쟁실험실.html` `L_HAPPYWORK`)과 그 A/B 계측기뿐이고,
//     `T157_HAPPYWORK=1` 이면 정본 `H` 가 걸린다. 랩·A/B 는 `world.happyWorkW` 로 덮어쓴다(값은 여전히 정본에서 읽는다).
...
if (_hwW > 0 && v.lastStats && typeof v.lastStats.happiness === 'number') {
  ...
  for (const npc of v.npcs) {
    ...
    const addProduce = (r, amt) => {
      ...
      amt *= _hpm * _hwm * _prodMul * sm * (v._laborMul || 1) * _siegeM * _capM;   // ★건강→작업량(±10%) × **행복→작업량(T157/T165 · 손잡이 끄면 1)** × production stat × 포만 × 부상노동력(요양=일손 X·부상=효율↓ — 생활층서 계산) × 포위 봉쇄(야외 직업만, 미설치=1) × 포로 노동(captive만 0.6, 미설치=1)
      if (amt <= 0) return;
  ...
  gardenFloorTopUp(v);   // ★[T100 5판 ⓒ] 텃밭 하한 — 하루 한 번(끄면 즉시 0 을 돌려준다 · 위 함수)
...
module.exports = {
  ...
  T157_HAPPYWORK_H, T157_HAPPYWORK, happyWorkWOf,   // ★[T165 이식] 정본 상수 하나 · 손잡이 · 마을별 계수 — 랩·계측기가 이 수를 옮겨 적지 않는다
  gardenFloorTopUp, T100_GARDEN, T100_GARDEN_CELLS, T100_GARDEN_FLOOR,   // ★[T100 5판 ⓒ] 텃밭 하한 — 값·손잡이·유도식을 하네스가 다시 계산한다
