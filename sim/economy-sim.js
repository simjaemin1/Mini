...
function consumeFood(v, need) {
  ...
  //     ★[T270 2026-09-13] 그 실측의 오늘 값은 이 함수 끝 주석에 있다(옛 줄은 안 지운다 · 족보).
  if (T73_RAWGRAIN) {
  ...
  //   ★★[T270 2026-09-13 · 근거를 오늘 수로] 위 한 줄은 **낡았다 — 지우지 않고 덧붙인다.**
  //     T263 실측(한반도 51마을 800일 · 식사 훅 켬): 인구 **−12.4 / −13.7 / −1.9%**(3시드 · T252 자로 **방향만**) ·
  //     **소멸 0**(세 팔 · 세 시드). 2026-07-12 의 −89%(마을이 죽었다)와 **크기가 다르다**.
  //     ⚠단 **다른 세계다** — 그때 근거는 CLI 앙상블(시드 101·202·303·505 · 6~8마을)이고 T263 은 실지도 51마을이다.
  //     ⇒ **판정은 유지**(방향이 같고 뒤집을 근거가 없다). 되돌림 손잡이는 `T263_FOOD_CONS`(기본 끔).
...
//   ⚠**기본 끔** — 손잡이가 없으면 아래 줄은 종전 `return` 한 줄뿐이라 **비트 동일**이다(T100·T193 env 관습 + 랩 `window` 관습).
function happyFloor1On() {
  const on = (x) => x !== undefined && x !== null && String(x) !== '' && String(x) !== '0';   // 빈 값은 **끔**(내보내기만 하고 안 채운 판을 켜면 안 된다)
  const g = (typeof window !== 'undefined') ? window : null;                       // 랩은 `window.L_HAPPY_FLOOR1` (lifeInit 전에 켠다 — 호출 때 읽는다)
  ...
  for (const npc of v.npcs) {
    ...
    if (jdef.produceSpecial === 'forager') {
      ...
      if (v.storage.food >= 1) {
        ...
        v.storage.food -= 1;
        // ★★[T258 2026-09-13] **부재료도 수요다** — 차감에 `_cons` 를 건다(바로 아래 보존식 가지가 쓰는 그 문법).
        //   T253 §0-ⓐ-2 실측: 이 줄엔 `_cons` 가 없어서 부재료 소비가 흐름-EMA(`_consEMA`)에 안 떴다.
        //   결과 셋 — v2 `tickDecay` 의 과잉부패 보호(`flowT = _consEMA × 30`)가 그 수요를 못 보고,
        //   가격 target 이 그 수요를 못 보고, 교역이 그 수요를 못 본다(`_consEMA.copper = 0` 이 만든
        //   "구리 써 본 적 없는 마을" 함정과 **같은 병**이다 · `:3489` 주석).
        //   ★새 수 0 — 소비량은 이미 있는 `0.5` 그대로다. ★기본 끔 — 켜면 세계가 움직인다(보고 T258 §0-ⓒ).
        const _sideCons = sideConsOn();
        for (const r of usedSides) { v.storage[r] -= 0.5; if (_sideCons) _cons(v, r, 0.5); }
...
module.exports = {
  ...
  HEALTH_PROD_W, happyWorkMul, happyFloor1On,   // ★[T157] 건강→작업량 계수 — 하네스가 "행복은 건강과 같은 문법" 을 대조한다(사본 0) · ★[T209] 하한 손잡이(하네스가 되돌림을 건다)
