...
global.__check=function(days){
  ...
  try{
  for(let t=0;t<days*1440;t++){lifeGM+=0.5;s.day=Math.floor(lifeGM/720);driveHunters(0.5);updateMobs(s,0.5);frames++;
    const f=(lifeGM%720)/720,tb=(f>=0.92||f<0.17)?'취':((f>=0.25&&f<0.833)?'주':'밤');
    TB[tb].h+=0.5/720*24;const kk=(s._kAll||0);TB[tb].k+=kk-pk;pk=kk;
    for(let hi=0;hi<s.agents.length;hi++){const a=s.agents[hi];if(a.job!=='hunter'||a._dead)continue;
      ...
      if(act==='저격'&&prevAct[hi]!=='저격'){const pp=prevPos[hi];if(pp&&Math.hypot(a.px-pp[0],a.py-pp[1])>0.05)fireWalk++;
        const tg=a._bm;if(tg&&(tg._spd||0)>=14)fireMove++;}
...
const C=[
 ...
 ['H6 무한 추적 없음(최장 ≤125분=포기 타임아웃 상한, 중앙값 ≤50분)', r.lock.maxDur/2<=125&&med()<=50, '최장'+(r.lock.maxDur/2).toFixed(0)+'·중앙'+med().toFixed(0)+'분'],
 ...
 ['H9 멈춤 교착(제자리 60분+ ≤3, 최장 ≤240분)', stuckOK(), r.stuckEp+'건·최장'+(r.stuckMax/2).toFixed(0)+'분'],
...
function med(){const d=r.lock.durs.slice().sort((a,b)=>a-b);return d.length?d[(d.length/2)|0]/2:0;}
function stuckOK(){return r.stuckEp<=3&&r.stuckMax/2<=240;}
