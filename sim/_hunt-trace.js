...
global.__run=function(sp,days){
  HUNT_SPECIES=sp;resetAll();lifeGM=432;_t0GM=lifeGM;
  const rows=[],kills=[];let prevK=0;
  for(let t=0;t<days*1440;t++){lifeGM+=1;s.day=Math.floor(lifeGM/1440);driveHunters(1);updateMobs(s,1);
...
console.log('위반 — 잠행-원거리(>85m):',vio.sneakFar+'틱 / 에피소드(≥2분)',ep.sneakFar,'/ 최장',(sfMax).toFixed(1)+'분',
...
console.log('멈춤(이동 행동인데 3m 격자 제자리 ≥30분): 에피소드',stuckEp,'· 최장',(stuckMax).toFixed(0)+'분');
const iv=[];for(let i=1;i<r.killT.length;i++)iv.push(r.killT[i]-r.killT[i-1]);
iv.sort((a,b)=>a-b);
...
console.log('도주 원인 —',JSON.stringify(r.fW));
