...
function tickSubsistence(v, day) {
  ...
  for (const [r, perNpc] of Object.entries(SUBSISTENCE_PER_NPC)) {
    ...
    // ★★[T288 · 장부 칸 한 줄] **이 인출은 여태 어떤 칸에도 안 남았다.** 그래서 목재 수지가
    //   200일 총유입의 20~22% 만큼 안 닫혔다(T279 §0ⓑ — 첫날 잔차 25.24 중 **20.00 이 이 줄**이다:
    //   50마을 × 8명 × 0.05 = 20.00, 소수점까지 맞는다). T130 §ⓐ 도 "매일 조용히 빼 가는 몫"이라 적고
    //   정본 `target`(= subs × 30)에서 **되읽는** 우회를 썼다 — 이 한 줄이 그 우회를 없앤다.
    //   ⚠**장부만이다.** `_cons` 도 EMA 도 가격도 사건도 안 건드린다(그건 세계 변경 · 재민 #37).
    //     읽는 쪽이 없으므로 손잡이도 없다 — 켜고 끌 것이 없다(값을 안 만드는 칸이라 비트 동일).
    //   ⚠누계다(하루치가 아니다). 문법은 정본의 `_herbUsed`·`_boneUsed`(누계) + `_foodEaten`(재화별 칸)을 합친 꼴.
...
function tickDecay(v) {
  ...
  for (const [r, baseRate] of Object.entries(DECAY_V2)) {
    ...
    // ★★[T288 · 장부 칸 한 줄] 위 `tickSubsistence` 와 **같은 구멍**이었다 — 썩은 양이 어디에도 안 남았다.
    //   차분(`s − 남은 것`)으로 적는다: `s × rate` 를 다시 곱하면 그게 사본이고 부동소수에서 어긋난다.
    //   ⚠장부만 — `_cons`·EMA·가격·사건 무접촉 · 손잡이 없음(읽는 쪽이 없다 · 비트 동일).
