...
// ═══ ★★★[T299 · ★재민 판정 2026-09-19 로 정정] 둘째 화물과 그 관문 — **기본 끔** ═══════════
//
// 이 절은 T299 가 "기본 켬"으로 앉혔던 자리다. **재민이 같은 날 끔으로 되돌렸다.** 근거는 T299 가 낸 표다:
//   · `twogate` 를 정당화한 근거는 꼬리였다(셋째 판 `<20명` 7곳 · 원안 `two` 소멸 3/15 · T256·T271).
//     **넷째 판에는 그 꼬리가 없다** — 끔 팔이 15시드 전부 `<20명` **0곳** · 소멸 **0/15** ·
//     최소 마을 중앙 36명이다. T244(손잡이 셋 기본 켬)가 이미 들어올렸다.
//   · 그래서 15시드 짝 Δ 는 **여덟 수 · 꼬리 두 집합 · 지니 전부 "못 가름"** 이다(보고/T299 §4).
//   · 15/15 로 남는 유일한 수는 **사건 밀도 ㉯ −7.44%**(2.010 → 1.870)인데, 그 값이
//     **밀도 캐논 띠(2~3일/건) 밖**이다 ⇒ 이롭다는 근거 0 · 캐논을 벗는 비용 1 ⇒ **끔.**
//   · `twobest`(T239) · 관문 셋(T280 ⓐⓑⓒ) 도 그대로 **끔**이다.
//
// 읽는 자리는 이 파일에 **하나**로 남긴다(자리는 T299 가 세운 그것이다 — 값만 뒤집혔다).
//   `server/villages.js` 에는 안 심는다 — T244 가 남긴 그 이유 그대로: 기준선의 자(`scripts/t17-metrics.js`)와
//   랩은 그 파일을 안 지나므로 거기 심으면 **서버만** 켜지고 표와 랩은 다른 세계를 잰다(T221 의 거울상).
//
// ⚠**미설정 = 끔**이고, 켜는 것은 **명시 `1`** 하나다 — `L_CARGO_TWO=1 L_CARGO_TWO_GATE=1`(env) ·
//   `window.L_CARGO_TWO=1`(랩). 손잡이를 안 주면 이 파일은 **다섯째 판과 비트 동일**이다(보고/T299 §7 · 3시드 `cmp`).
//   켠 팔(`=1` 둘)은 보고/T299 §3 의 여섯째 판 수를 그대로 다시 낸다 — 표를 버리지 않는다.
// ⚠명시 주입(`world.cargoTwo` · `world.cargoTwoGate`)이 **가장 세다** — A/B·계측기가 팔을 손으로 잡는다.
//   `false` 주입도 존중한다 — 그래서 `undefined` 검사지 진리값 검사가 아니다.
// ⚠**새 수 0** — 이 절은 문의 기본값만 정한다. 용량·후보·관문식은 한 글자도 안 건드렸다.
...
function cargoTwoOn(world) {
  ...
  const x = _cargoKnob('L_CARGO_TWO'); return x !== null && x !== '0';                  // 미설정 = 끔(기본 · 재민 09-19)
}
function cargoTwoGateOn(world) {
  if (world && world.cargoTwoGate !== undefined && world.cargoTwoGate !== null) return !!world.cargoTwoGate;
  const x = _cargoKnob('L_CARGO_TWO_GATE'); return x !== null && x !== '0';             // 미설정 = 끔(기본 · 재민 09-19)
}
...
function computeShadowPrices(v) {
  ...
  for (const r of TRADABLE) {
    ...
    if (CAP_TARGET[r] !== undefined) {
      ...
      const flowT = subsGuard(r, 'price') ? 0 : ((v._consEMA || {})[r] || 0) * 30;
...
function _priceParamsV2(v, r) {
  ...
  if (r === 'tool' || r === 'bronze_tool' || r === 'iron_tool') {
    ...
    const subs = (SUBSISTENCE_PER_NPC[r] || 0) * N;
    const flowT = subsGuard(r, 'price') ? 0 : ((v._consEMA || {})[r] || 0) * 30;   // ★flow-EMA(동기 계약: computeShadowPrices와 동일 공식·subs 가드)
...
function tickTradeV2(world, day) {
  ...
  for (const a of data) {
    ...
    while (caravansLaunched + currentlyTrading < spareCap) {   // 여유노동까지만. 그 안에서 잉여·기회비용이 추가 제약.
      ...
      // ★★★[T206 2026-09-12 · 이 파일의 접점 ①/② 중 ①] **남는 용량에 둘째 후보.**
      //   §0-ⓐ 실측(시드 1020 · 8,152 leg): 적재율 중앙 92%(바닥 마을 82.8%) · 만재는 46.6% 뿐이고
      //   **빈자리가 있는데 둘째 후보도 있던 leg 가 48.8%**(3,979건 · 더 실을 수 있던 양 124,134 단위).
      //   T173(귀환 화물 하나)·T200(출발 화물 하나 · 품목 간 대체 · 족보 161)이 만난 그 벽이 이 한 줄이다.
      ...
      if (cargoTwoOn(world)) {        // ★[T299 · 재민 09-19] 기본 **끔** — 켜는 것은 `L_CARGO_TWO=1` 하나(주입이 가장 세다)
        ...
        if (_room >= 1) {
          ...
          for (const c2 of candidates) {
            ...
            if (_c2gate && !(_pu2 * _q > 0)) { if (!_picked) _gateBlocked++; continue; }   // ★걸리면 **다음 후보**로 · ★[T299] 기본 끔
            ...
            if (_tot > _bTot) { _bTot = _tot; _bRes = c2.res; _bN = _q; _bP = a.prices[c2.res] || 0; _bPP = _pu2; }
            //   ⚠**여기서 멈추지 않는다.** 멈추면 `_bTot` 이 첫 개만 보게 되어 갈림이 늘 0 으로 나온다
            //     (첫 판이 그렇게 0 을 냈다). 훑기는 읽기뿐이라 고르는 값이 안 바뀌므로 비트는 그대로다 —
            //     `_res2`·`_n2`·`_p2`·`_pp2`·`_gateBlocked` 는 전부 `_picked` 로 잠겨 한 번만 쓰인다.
          }
          //   ★★★[T239] **수익 최대 문** — 첫째는 목적지 20곳 × `best.profit` 최대로 고르는데
          //     둘째는 선언 순서였다(T231 §ⓐ 의 마지막 비대칭). 같은 자로 고르게 한다 — 새 수 0.
          //     셋째는 여전히 0(고르는 건 하나뿐이다).
          if (world.cargoTwoBest && _bRes) { _res2 = _bRes; _n2 = _bN; _p2 = _bP; _pp2 = _bPP; }
        }
      }
      if (_res2 && _n2 >= 1) {
        a.v.storage[_res2] -= _n2;                    // 출발 즉시 차감(첫 품목과 같은 자리·같은 문법)
        a.sent += _n2 * _p2;
        { const r = _res2, base = BASE_VALUE_V2[r] || 1, elast = ELASTICITY[r] || 1,
            subs = (SUBSISTENCE_PER_NPC[r] || 0) * N, buf = N * Math.max(0.5, (UTILITY_WEIGHT[r] || 0.1) * 1.2),
            tg = Math.max(subs * 30, buf), st = Math.max(0.1, a.v.storage[r] || 0);
          a.prices[r] = base * Math.max(PRICE_ADJ_MIN, Math.min(PRICE_ADJ_MAX, Math.pow(tg / st, elast))); }
      }
      ...
      world.caravans.push({
        ...
        giveRes2: _res2 || null, giveAmt2: _n2 || 0, pFrom2_at_depart: _p2 || 0,   // ★[T206] 둘째 화물(미주입이면 null/0)
      ...
      // ★★[T206 2026-09-12 · **계측 전용 문** — 주입이 없으면 한 글자도 안 바뀐다] 출발 leg 한 건의 자.
      //   같은 벽을 두 번 만났다(T173 귀환 화물 · T200 출발 화물): **캐러밴 하나 = 품목 하나**이고,
      //   잉여가 `CARGO_PER_TRIP` 에 못 미치면 수레가 **빈 채로** 간다. 그 빈 자리를 재려면
      //   `candidates` 를 밖에서 봐야 하는데, 문턱 규칙을 계측기에 다시 적으면 그게 사본이다(사본 0).
      ...
      if (world.onTradeLeg) {
        try {
          ...
          world.onTradeLeg({ day, vid: a.i, from: a.v.name, to: b.v.name,
            ...
            //   ★[T239 관측 항] 수익이 최대였을 후보 — 손잡이를 켜지 않고도 갈림을 센다.
            p2Best: _bRes, p2BestUnits: _bN, p2BestTotal: (_bTot === -Infinity ? 0 : _bTot),
            p2Total: (_pp2 || 0) * (_n2 || 0),
      ...
      if (a.v.tradeStats) {
        ...
        if (_res2 && _n2 > 0) eb[_res2] = (eb[_res2] || 0) + _n2;   // ★[T206] 둘째 화물도 같은 감사 열에
        if (_res2 && _n2 > 0) a.v.tradeStats.cargoSent += _n2;
      }
    } // while
  } // for (const a of data)
}
    ...
    if (c.state === 'outbound' && day >= c.arriveDay) {
      ...
      if (expectedRevenue < sunkCost * 0.5 && rerouted < 2) {
        ...
        if (bestAlt) {
          ...
          if (c.giveRes2 && c.giveAmt2 > 0) c.giveAmt2 = c.giveAmt2 * (1 - outboundLoss);   // ★[T206] 둘째도 같이 간다
          world.tradeLog.push({
        ...
        // ★[T206] 둘째 화물도 **집으로 돌아온다** — 안 적으면 그 양이 세계에서 사라진다(질량 누수).
        if (c.giveRes2 && c.giveAmt2 > 0) { c._returningRes2 = c.giveRes2; c._returningAmt2 = c.giveAmt2 * (1 - outboundLoss); }
        c._abandoned = true;
        c.state = 'inbound';
        ...
        // ★★[T214 2026-09-12 · 재민 확정] `departDay` 는 **가는 구간**의 값이다 — 여기서 옮기면 안 된다.
        //   재routing(위)은 **여전히 outbound** 라 `arriveDay`·`departDay`·`travelDays` 셋을 같이 옮기는
        //   게 맞다. 빈손 귀환은 `state` 가 **inbound** 로 바뀌고, 귀환 구간을 읽는 자리는 전부
        //   `arriveDay → returnArriveDay` 다(`central.js:669-674,690` · `villages.js:1984`) —
        //   inbound 에서 `departDay` 를 읽는 자리는 **한 곳도 없다**(전수 grep). 그런데 여기서
        //   `day` 를 써 넣으면 `arriveDay − departDay` 만 0 이 되어 T60 ③ 의 세 값 규약이 깨진다.
        //   실측(3시드 240일 · 버림 13건): 전 `arr−dep === travelDays` ✓ → 후 `arr−dep = 0` ✗ ·
        //   `returnArriveDay − arriveDay === travelDays` 는 전후 모두 ✓ · 거리는 대칭이라
        //   `travelDays` 는 값이 안 바뀐다(620.58→620.58 …). ⇒ 움직인 것은 이 한 줄뿐이었다.
        //   `test-events ⑲c` 가 잡은 것이 이것이다(T185 가 `363c38cb` 로 이분할한 그 빨강 —
        //   그 커밋은 **표본이 이 갈래를 처음 밟게** 했을 뿐, 결함은 빈손 귀환이 생긴 이래 있었다).
      ...
      // ★★[T206 · 접점 ②] 둘째 화물 매도 — **첫 품목과 완전히 같은 문법을 한 번 더** 쓴다.
      //   (충격 정산 `_impactSellV2` · 원석 조성 폴드 · 재고 가산 · 세금 — 순서까지 같다.)
      //   미주입 세계에는 `giveRes2` 가 없으므로 이 블록도 통째로 건너뛴다 = 비트 동일.
      let _gross2 = 0, _delivered2 = 0;
      if (c.giveRes2 && c.giveAmt2 > 0) {
        _delivered2 = c.giveAmt2 * (1 - outboundLoss);
        _gross2 = _impactSellV2(c.to, c.giveRes2, _delivered2);
        if (c.giveRes2 === 'ore' && v1.foldOreMix) v1.foldOreMix(c.to, c.to.storage.ore || 0, _delivered2, v1.oreMixOf(c.from));
        c.to.storage[c.giveRes2] = (c.to.storage[c.giveRes2] || 0) + _delivered2;
      }
      const taxTo = (grossCredit + _gross2) * TAU;
      const netCreditAfterArrival = (grossCredit + _gross2) - taxTo;
      ...
      // ★★★[T173 2026-09-11 · 이 파일의 유일한 접점] **귀환 화물 우선순위를 밖에서 물어본다.**
      //   여기가 "대금으로 무엇을 실어 오는가"를 정하는 자리다 — 지금 규칙은 `ratio` 최대 하나뿐이고,
      //   예외는 **식량 pull**(위 P1b) 하나다. 식량은 굶으면 끌어오는데 **돌은 그런 자리가 없다.**
      //   T173 §0-ⓑ 실측: 석재 바닥 36마을이 800일 동안 받은 돌이 **0**(파는 마을은 13곳 · 판 것은 11,723).
      //
      //   ⚠**주입이 없으면 한 글자도 안 바뀐다.** 규칙은 이 파일이 안 갖는다 — `world.returnPullFn` 이 준다.
      //     지금 그것을 주는 것은 **랩**(`lab/전쟁실험실.html` `L_STONE_TRADE`)뿐이고, 서버·CLI·하네스는
      //     안 주므로 폴백이 곧 종전 = **비트 동일**(T135 `forageTakeFn` · T157 `happyWorkW` ·
      //     T161 `allocFn` · T163 `stoneBudgetFn` 선례).
      //   ⚠**문은 규칙을 느슨하게 하지 않는다** — 파는 쪽에 실물이 있고(>1) 받는 쪽이 글럿이 아닌
      //     경우에만 갈아 끼운다(위 두 게이트를 그대로 다시 건다 · 공짜 돌 0).
      {
        const _rpFn = (world && typeof world.returnPullFn === 'function') ? world.returnPullFn : null;
        if (_rpFn) {
          let _r = null;
          try { _r = _rpFn(c.from, c.to, returnRes); } catch (e) { _r = null; }
          if (_r && _r !== c.giveRes && (c.to.storage[_r] || 0) > 1 && !_returnGlutted(_r)) returnRes = _r;
        }
      }
      ...
      // ★[T206] 빈손 귀환으로 되돌아온 **둘째 화물** — 첫 품목과 같은 문법(세금은 안 붙는다: 안 판 짐이다).
      if (c._returningRes2 && c._returningAmt2 > 0) {
        const received2 = c._returningAmt2 * (1 - inboundLoss);
        if (c._returningRes2 === 'ore' && v1.foldOreMix) v1.foldOreMix(c.from, c.from.storage.ore || 0, received2, v1.oreMixOf(c.to));
        c.from.storage[c._returningRes2] = (c.from.storage[c._returningRes2] || 0) + received2;
      }
      c._done = true;
    }
  }
...
function tickDecay(v) {
  ...
  for (const [r, baseRate] of Object.entries(DECAY_V2)) {
    ...
    const subs = (SUBSISTENCE_PER_NPC[r] || 0) * N;
    const util = UTILITY_WEIGHT[r] || 0.1;
    const flowT = subsGuard(r, 'price') ? 0 : ((v._consEMA || {})[r] || 0) * 30;   // ★flow-EMA(동기 계약·subs 가드) — 롱테일 작업 재고는 과잉부패에서 보호
...
// ════════════════════════════════════════════════════════════════════════════════════════
// ★★[T416 2026-09-26] **subs 가드 — 술어 하나**(#63 · 두 기계가 같은 문을 지난다 · 값 무변)
//   같은 소비 흐름(`v._consEMA`)을 두 기계가 읽는다:
//     · **가격**(`computeShadowPrices` · `_priceParamsV2` · `tickDecay` — 동기 계약 셋) — `flowT = 가드 ? 0 : EMA×30`
//       가드 유래: 2026-07-12 flow-EMA 채택 세션(문서/아카이브/CHECKLIST.md:161 ②) — "SUBSISTENCE_PER_NPC 등재 재화는
//       subs×30 이 이미 손튜닝 흐름 target: flowT 중복 시 … 캘리브를 2~3배 덮어써 시장 유보 인플레(s505 576→17 붕괴·가드로 542 회복)".
//     · **게시판**(`server/events.js` 부족·과잉 래치 → 의뢰) — `ema > 0 && stock < ema × SHORT_DAYS`. 가드가 **없었다**.
//       그땐 식량이 EMA 에 아예 안 들어왔다(같은 날 ① "식단 폴드 금지") — 게시판(08-25)이 가드를 물을 일이 없었다.
//       `T263_FOOD_CONS=1` 이 식량을 EMA 에 넣으면 둘이 갈린다(T362 §3: 가격 5종 · 게시판 14종).
//   ⇒ 판정 자리는 이 술어 **하나**다. 두 기계는 이것을 부르고 규칙을 다시 적지 않는다(사본 0).
//   ⚠기본은 **현행 그대로**(가격 가드 · 게시판 무가드) = 넷째 판 비트 동일. 팔은 `T416_SUBS_GUARD`:
//     `a` = 둘 다 가드(생존 품목은 곳간이 댄다 — 게시판도 안 다룬다) · `b` = 둘 다 무가드(부족은 의뢰로 · 가격도 flowT).
//     ★어느 팔이 정본인가는 재민 #63 — 이 술어는 **켜지 않는다**.
function isSubsistenceFlow(r) { return !!SUBSISTENCE_PER_NPC[r]; }
const _T416_ARM = (typeof process !== 'undefined' && process.env && process.env.T416_SUBS_GUARD) ? String(process.env.T416_SUBS_GUARD).toLowerCase() : '';
