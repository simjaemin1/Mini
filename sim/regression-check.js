...
// ★픽스처 교체 2(2026-07-12): 8→202. s8 맵은 금일 11개 독립 섭동 중 7개 사망(식량 증가 순개선 포함 —
//   널 ×1/1000·봉헌 v2는 생존 = 무한 칼날은 아니나 건강 분지가 극히 좁은 한계 맵) — 픽스처로서 분별력 없음
//   (모든 econ 변경의 검증을 막는 지뢰밭). 202는 무제한(461/7)·캡(780/8)·충격정산(668/7) 3개 econ 변형
//   @1000f에서 전부 건강 — 강건 맵. seed19→404 교체와 동일 원칙(픽스처=분별력 있는 시드).
const SEEDS = [7, 42, 202, 3, 404];
...
Promise.all(jobs).then(parts => {
  ...
  const checks = [
    ...
    ['17. 위신재 교역 활성(장식 > 12000)', agg.tradeOrn > 12000],   // ★재캘리브(2026-07-12, 시장충격 정산 9e2430f): 위신재는 target이 작아(N×LUX_TARGET_PC) 충격 정산이 흡수 한계를 올바르게 걸어 균형 유통량 하락 — 구 100k는 flat 정산(~300k) 기준. @1500 5시드 직렬 실측 36,693(시드별 2,495~10,524 전원 활성) → 임계 = 신균형의 ~1/3(원캘리브와 동일 3× 검출 여유). 병리 참조점: 위신경제 죽은 세계 ~278(s19 사망 실측) — 검출력 유지
