  ...
  function updateHud() {
    ...
    if (wb) {
      if (myWeather) {
        ...
        // ★[T105 2026-09-05] **한 낱말**만 얹는다 — 새 패널도, 새 게이지도 없다.
        //   젖음의 결과(단열이 준 것)는 아래 툴팁의 `insC` 가 **이미 젖은 값**이라 저절로 참말이 된다.
        const wet = Math.max(0, Math.min(1, myWeather.wet || 0));
        // ★[T114 2026-09-05] **한 낱말 더** — 비가 올 때만, 가려 주는 자리에서만.
        //   안 올 때 띄우면 "나무 아래"가 지형 설명이 돼 버린다 — 이건 **지금 덜 젖는다**는 말이다.
        //   문턱은 마을 낱말(`sh > 0.15`)과 같은 눈금을 쓴다(새 문턱 0). 계수 사본 0 — 서버 `cover` 그대로.
        const cov = Math.max(0, Math.min(1, myWeather.cover || 0));
        const rain = (myWeather.precip || 0) > 0.01;
        const txt = ` ${myWeather.ko}${sh > 0.15 ? ' · 마을' : ''}${rain && cov > 0.15 ? ' · 나무 아래' : ''}${wet > 0.01 ? ' · 젖음' : ''}`;
