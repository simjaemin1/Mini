// @@split:34-m-renderloop — M/R2 — 렌더 루프(render)·인접 존 방향 화살표 (T51 ③)
  // === 렌더링 (아이소메트릭) ===
  function render() {
      ...
      if (item.kind === 'claim') {
        ...
        if (cl.facilityType && _claimCellSeen(Math.floor((off + cl.x + cl.w / 2) / _S0), Math.floor((offY + cl.y + cl.h / 2) / _S0))) {
          ...
          if (cl.facilityType === 'farmland' && cl.farmStage != null) {
            ...
            // ★[T101] 앵커가 정본이다 — 델타 계산 0(`drawImage(im, x-_ox, y-_oy)` 한 줄).
            if (_cs) { ctx.drawImage(_cs, cs.x - _cs._ox, cs.y - _cs._oy); noRender = false; }
          }
          if (noRender) {
    ...
    // === 4-0) 날씨(비·눈) — **안개 위 · 밤 오버레이 아래** (T93 · 37-r1-weather.js) ===
    //   위: 셀에 붙은 사물이 아니라 카메라와 세상 사이의 공기라 안개 마스크에 안 잘린다.
    //   아래: 밤이 비를 같이 어둡게 덮어야 한다(빗줄기만 훤한 건 공기가 아니다).
    drawWeather(ctx, W, H, performance.now());
