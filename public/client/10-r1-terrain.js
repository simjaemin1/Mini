...
// @@moved:2234
  ...
  // ★★[T150 2026-09-07] **둥글기 축** — 재민 2026-08-07 *"한반도는 둥근 거 위주로"*.
  //   ⚠**기본 0 = T150 이전과 비트 동일.** 켜는 건 재민 눈 판정 뒤다(`__mt3SetRound`).
  //   둥근 = 완사면이다 ⇒ 경사 길이 `lam` 을 **늘리고** 마루 높이 `hmax` 를 **낮춘다**.
  //   ★두 배수는 **새 수가 아니다** — 스프라이트 재료표(`scripts/bake-mountain.py`)의 평균비다.
  //     그래야 3D 판과 스프라이트 판(`mt_R*` 40장)이 **같은 모양 언어**를 쓴다.
  //       뾰족 40종 평균 : 밑변 2.44 · 키 4.66      (mt_G 24 + mt_F 16)
  //       둥근 40종 평균 : 밑변 3.13 · 키 3.35      (mt_RG 24 + mt_RF 16)
  //     `test-icons` 가 이 두 수를 **재료표에서 다시 계산해** 대조한다(사본이 굳는 것을 막는다).
  let MT3_ROUND = 0;                   // 둥글기 비율 r ∈ [0,1] — 0 이면 아래 곱이 통째로 안 돈다
  const MT3_RLAM = 3.13 / 2.44;        // 1.2828 — 둥근 산의 경사 길이 배수
  const MT3_RHMAX = 3.35 / 4.66;       // 0.7189 — 둥근 산의 마루 높이 배수
  ...
  function _mt3Field(zid, gx, gy) {
    ...
    for (let j = 0; j < N; j++) for (let i = 0; i < N; i++) {
      ...
      let lamL = MT3_LAM * Math.exp(MT3_LAMV * (2 * _mt3vn(ai / 26, aj / 26, 53) - 1));
      // 자리마다 다른 마루 높이 — 봉우리와 안부(鞍部)가 생긴다.
      let hmaxL = MT3_HMAX * (1 - MT3_HV * 0.5 + MT3_HV * _mt3vn(ai / 37, aj / 37, 59));
      // ★★[T150] 둥글기 — **자리마다 이어진 장**이지 셀마다 던지는 동전이 아니다.
      //   ⚠카드는 "자리마다 둥근/뾰족은 `_cellHash`"(T145 그대로)라고 했지만, **높이장에서는
      //     그러면 안 된다.** 스프라이트 판은 산 하나가 제 실루엣을 가진 **따로 선 그림**이라
      //     이웃이 뾰족이든 둥글든 상관없다. 여기는 **이어진 면**이다 — 둥근 셀과 뾰족 셀이
      //     맞닿으면 그 한 셀에서 높이가 통째로 갈린다(실측: 최대 |Δh| 10m 대의 수직 벽).
      //   ⇒ 이진 문턱 대신 **부드러운 장** q ∈ [0,1] 을 쓴다. 척도는 새로 안 짓고 `lam` 변주와
      //     같은 26셀을 쓴다(둥글기는 경사 길이의 성질이니 같은 척도가 맞다).
      //   ★q = vn^((1−r)/r) — **넓이 평균이 r 이 되는 사상**이다(균등 잡음이면 정확히 r).
      //     문턱(vn < r)은 안 된다: `_mt3vn` 은 네 해시의 보간이라 **종 모양**이고(5% 분위 0.136),
      //     r=0.8 로 자르면 넓이의 **90.1%** 가 둥글어진다 — 표에서 유도한 0.8 이 조용히 0.9 가 된다.
      //     지수 사상은 같은 조건에서 0.8195 를 낸다(실측 · 오차 1.9%p · r=0.5 에선 정확).
      if (MT3_ROUND > 0) {
        const q = Math.pow(_mt3vn(ai / 26, aj / 26, 89), (1 - MT3_ROUND) / MT3_ROUND);
        lamL *= 1 + q * (MT3_RLAM - 1);
        hmaxL *= 1 + q * (MT3_RHMAX - 1);
      }
