...
(() => {
  ...
  async function boot() {
    ...
    canvas.addEventListener('mousemove', (e) => {
      ...
      // 14.51 + 14.53-e + 14.53-g: hover list (cell 안 여러 building) — 휠로 cycle 가능
      if (buildMode && !placementMode) {
        ...
        for (const c of conns.values()) {
          ...
          for (const b of c.buildings.values()) {
            ...
            const ax = ox + b.x + (isEdge ? 16 : 0);
            const ay = oy + b.y + (isEdge ? 16 : 0);
            const d = Math.hypot(ax - wx, ay - wy);
            // 32px 안 — 같은 cell + 이웃 edge까지 포함
            if (d < 32) candidates.push({ id: b.id, d });
          }
        }
        candidates.sort((a, b) => a.d - b.d);
        const newList = candidates.map(c => c.id);
