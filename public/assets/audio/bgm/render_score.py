...
from render_articulation import XFADE, articulation_render_options, plan_sustained_score
...
def build(spec=None):
    ...
    # 대금의 연결은 더 이상 ``< 20 ms`` 같은 시간 창으로 추정하지 않는다.
    # 점수표에 slur/tie를 명시한 자리만 `notes`의 true가 되고, 4장단 악구
    # 시작은 새 숨이다. 기본 score에는 아직 확정한 slur가 없으므로 아래 map은
    # 비어 있어도 된다 — "표기 없음 = 재발음"이 정본이다.
