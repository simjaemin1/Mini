...
(async () => {
  ...
  await sleep(1500);
  const _zone2 = boot('zone', 'zone.js', { PORT: String(ZPORT), ZONE_ID: 'hanbando', DB_PATH: ZDB, CENTRAL_URL: `http://localhost:${CPORT}`,
    VILLAGE_MAX: '2', ENABLE_BANDITS: '0', ENABLE_ROADS: '0', ENABLE_WILDLIFE: '0', E2E_GIVE: '1',
  ...
  ok(await (await _zp2).ok, '만조로 zone 재기동');
  await page.reload({ waitUntil: 'domcontentloaded' });
  await sleep(2500);
  const enter2 = await page.$('#enter');
