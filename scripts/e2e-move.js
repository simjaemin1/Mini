...
const metric = async (k) => {
  try { const t = await (await fetch(`http://localhost:${ZPORT}/metrics`, { signal: AbortSignal.timeout(5000) })).text();
...
async function runPhase(model, SPOT, deep) {
  ...
  ok(_up.ok, `[${model}] central 기동`, _up.ok ? `${_up.ms}ms · 아이가 제 입으로 말했다` : _up.why);
  ok(await (await _zp).ok, `[${model}] zone 기동`);
