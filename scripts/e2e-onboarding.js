...
(async () => {
  ...
  await browser.close();
  shutdown();
  // ★★[T346 2026-09-22] **판정 수는 게이트가 아니다.** 이 대본의 판정 개수는 세계에 따라 사람마다 ±1 흔들린다:
  //     갈래 ⓐ 촌장에게 걸린 첫 의뢰가 있나 — 있으면 판정 둘(제안 + 방향), 없으면 하나(쉼터 권유)  ← 위 §④ 갈래
  //     갈래 ⓑ 첫 납품이 성립했나 — 성립하면 판정 둘(납품 알림 + 곳간 이펙트)이 **더** 붙는다      ← 위 §⑤ round 0
  //   둘 다 **게시판이 실제로 낸 의뢰**의 함수다(대본이 지어내지 않는다 — §④ 머리 주석).
  //   ⇒ 불변량은 **FAIL 0** 하나다. 절대 수("75/0")로 회귀를 판정하면 이 갈래를 결함으로 읽는다
  //     (T346 실측: 같은 대본이 66/5 · 74/0 · 75/0 을 다 냈다 — 앞의 하나만 진짜 빨강이었다).
  console.log(`\n── 판정 ${pass + fail}개(갈래 ⓐ 의뢰 유무 · ⓑ 첫 납품 성립에 따라 사람마다 ±1) — 게이트는 FAIL 0 하나다 ──`);
  console.log(`\n=== PASS ${pass} / FAIL ${fail} ===`);
