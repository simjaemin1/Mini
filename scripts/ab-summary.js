...
for (const tag of tags) {
  ...
  let ex = 0, exN = 0, cashPc = 0, slope = 0, slopeN = 0;
  ...
  for (const s of SEEDS) {
    ...
    for (const v of d.villages) {
      ...
      if (v.expansions != null) exSeen = true;
      sEx += v.expansions || 0;
      sCash += (v.treasury && v.treasury._cash) || 0;
    ...
    // ★현금 기울기 — 궤적에서 800일 시점과 마지막 표본의 **1인당 현금**을 뽑아 하루당 증가율을 낸다.
    //   (마을별 궤적을 세계 합으로 다시 모아야 한다 — 마을 하나만 보면 소멸·이주로 흔들린다)
    {
      const at = (day) => { let c = 0, pp = 0, hit = 0;
        for (const v of d.villages) { const h = v.history || []; let best = null;
          for (const x of h) if (x.d <= day && (!best || x.d > best.d)) best = x;
          if (best && best.cash != null) { c += best.cash; pp += best.p; hit++; } }
        return hit && pp > 0 ? c / pp : null; };
      const dEnd = d.days || 800;
      const a = at(Math.min(800, Math.floor(dEnd / 2))), b = at(dEnd);
  ...
  rows.push({ tag, pop: +(pop / n).toFixed(0), ext: +(ext / n).toFixed(2), zomb: +(zomb / n).toFixed(2),
    ...
    cashPc: +(cashPc / n).toFixed(1),
    slope: slopeN ? +(slope / slopeN * 1000).toFixed(1) : null,   // 1,000일당 1인당 현금 증가
    per, missing });
}
