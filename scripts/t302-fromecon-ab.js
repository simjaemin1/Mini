#!/usr/bin/env node
// === scripts/t302-fromecon-ab.js — T302: `fromEcon` 다섯 줄 반사실(같은 세계 · 장부 둘) ==========
//
// ⚠**계측기다. 하네스가 아니다 — 러너에 넣지 마라**(`@regress` 없음).
// ⚠세계는 `t274-board-ab.js` 와 **같은 문**(족보 130 · 사본 0) — 그 파일에서 갈라져 나왔다.
// ⚠**소스 치환 0** · **레포 코드 0**: 이 계측기가 재는 것은 `villages.playerVillageDepositMap()` 이
//   장부에 **주입되는 지점** 하나다. 주입을 바꾸는 것은 계측기 안이고, 정본 파일은 무접촉이다.
//
// 무엇을 묻나 — T302 가 표에 더한 다섯 줄(`salmon`·`shrimp`·`crab`·`oyster`·`seaweed`)이
...
//
// 실행: node scripts/t302-fromecon-ab.js [일수=800] [시드=1020]
//   FIVE=1|0   1 = 정본 표 그대로(다섯 줄 있음 · 이 카드 뒤) · 0 = 다섯 줄을 뺀 반사실(이 카드 전)
...
const FIVE_ON = process.env.FIVE !== '0';
const DEPOSIT = (() => {
  const m = Object.assign({}, Villages.playerVillageDepositMap());
  const 있다 = FIVE.filter((r) => m[r] === r).length;
  if (있다 !== 5) { console.error(`정본 표에 다섯 줄이 없다(${있다}/5) — 이 계측기는 T302 뒤의 표를 전제한다`); process.exit(2); }
  if (!FIVE_ON) for (const r of FIVE) delete m[r];
  return m;
})();
...
console.log(`\n=== fromEcon 다섯 줄 반사실 — 서버 ${seeds.length}곳 · 시드 ${SEED} · ${DAYS}일 · 다섯 **${FIVE_ON ? '있음(T302 뒤)' : '없음(T302 전)'}** ===`);
...
}
{
  ...
  console.log(`    ★다섯(T302) — 낼 수 있나 ${FIVE.filter((r) => _DEL.fromEcon.has(r)).length}/5 · 벽에서 빠졌나`);
  for (const r of FIVE) {
...
console.log(`  손잡이    FIVE=${FIVE_ON ? '1(정본 표 그대로)' : '0(다섯 줄 뺀 반사실)'} — 정본 파일 무접촉 · 소스 치환 0`);
if (process.env.AB_JSON) {
  ...
  fs.writeFileSync(process.env.AB_JSON, JSON.stringify({ five: FIVE_ON, wall: LADDER.filter((r) => !_DEL.fromEcon.has(r)), tally: TALLY, seed: SEED, days: DAYS, track: TRACK, sides: SIDES, nLive: nLive2,
