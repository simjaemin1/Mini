...
//   그리고 여전히 두 강이 만나는데 끊겨 있는 곳이 존재한다. 옥돌천이 만나는 곳도 어색하다."
//   → 전수 실측으로 셋 다 확인됐다.
//
...
// ② 두 강이 만나야 하는데 수십 셀 벌어져 끊긴 곳 — 실측 2건(맑바람천↔옥돌천 36셀, 쇠벼루천↔푸른솔천 35셀).
...
// (살여울천↔율호가 그랬다) — 그래서 기본을 0.4셀로 조인다.
...
//   실제로는 내륙인 끝까지 바다행으로 오인해 건너뛰었다(흰숲천 끝은 아래 경계에서 2,368px인데 넘어갔다).
...
// ★재민 지적: "살여울천 다시 엄밀하게 봐. 어디가 상류야? 상류→하류 굵어져야 해."
//   실측: 살여울천은 양 끝이 133(상류) / 371(하구)로 맞는데 **중간이 475로 불룩**했다(감소 구간 8개).
...
for (const rv of rivers()) {
  ...
  if (wetS && !wetE) mouthEnd = 0;
    ...
    //   여기에 단조를 강요하면 강이 통째로 눌린다(옥돌천: 양끝 120인데 중간 봉우리를 다 깎아 평평해졌다).
