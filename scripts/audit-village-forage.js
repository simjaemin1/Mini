...
//   + **식수 접근 1개**.  15초 = MOVE_SPEED(64px/s) × 15 = **960px = 30셀**.
...
function auditVillage(v) {
  ...
  return { name: v.name, type: v.type, x: v.x, y: v.y, cnt, near, bushes,
           stand, water, okKind, okWater, pass: KINDS.every((k) => okKind[k]) && okWater };
}
...
console.log(`        시작 재료 3종 각각 소스 ≥ ${NEED_EACH}개 + 식수 1개.  (직선거리 대리 — 합격선이지 체감 시간이 아니다)`);
...
const head = `  ${padr('마을', 10)}${padr('종', 5)}│ ${pad('잔가지', 7)}${pad('자갈', 6)}${pad('풀', 5)} │ ${pad('덤불', 5)} │ ${pad('물', 6)} │ 최근접(잔가지/자갈/풀)`;
const line = (r) => `  ${padr(r.name, 10)}${padr(r.type || '', 5)}│ ${pad(r.cnt.twig, 7)}${pad(r.cnt.pebble, 6)}${pad(r.cnt.fiber, 5)} │ ${pad(r.bushes, 5)} │ ${pad(Number.isFinite(r.water) ? Math.round(r.water) : '없음', 6)} │ ` +
...
if (fail.length) {
  const missing = { twig: 0, pebble: 0, fiber: 0, water: 0 };
  for (const r of fail) { for (const k of KINDS) if (!r.okKind[k]) missing[k]++; if (!r.okWater) missing.water++; }
  console.log(`   부족한 재료: ` + KINDS.map((k) => `${KO[k]} ${missing[k]}곳`).join(' · ') + ` · 식수 ${missing.water}곳`);
