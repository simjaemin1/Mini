...
//   또 하나 방증: design-region.js 는 청류대천에 `_connect: '맑내천(N) 연장'` 이라고 적어 뒀다 —
...
//   달재천 ↔ 아카가와는 끝점이 3셀로 붙어 있어 "이어 그은 획"처럼 보였지만, 실제로는 **한 점에서 갈라져
...
for (const chain of chains) {
  ...
  for (const ri of chain) {
    ...
    //   이음매가 **이미 쌓아 둔 경로의 머리 쪽**일 수 있다. 대숲림천이 그랬고(이음매=path[0]),
    //   살여울천을 꼬리에 붙이는 바람에 14,508px(453셀)을 폭 475로 가로지르는 직선이 생겼다.
