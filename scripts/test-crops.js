...
(async () => {
  ...
  // ── ★★⑧ [T58b] 플레이어 돌보기 · 품질 · 빈 밭 ─────────────────────────────
  {
    ...
    const codeOnly2 = require('./code-only.js');   // ★[T171] 주석 제거기 **정본**(acorn onComment · 사본 0). 옛 정규식 판은 `villages.js:20` 의 `// … sim/* …` 에 걸려 파일의 67.9% 를 삼켰다
  ...
  // ── ★★★⑫ [T99] 월동 — 겨울을 지나야 익는다 · 겨울엔 돌봄도 쉰다 ───────────────
  {
    ...
    {
      const fs3 = require('fs');
      const codeOnly3 = require('./code-only.js');   // ★[T171] 주석 제거기 **정본**(acorn onComment · 사본 0). 옛 정규식 판은 `villages.js:20` 의 `// … sim/* …` 에 걸려 파일의 67.9% 를 삼켰다
  ...
  // ── ★★★⑬ [T108] 옛 벽시계 밭 철거 · `RESOURCE_HP` 사본 제거 ───────────────────
  {
    ...
    const codeOnly4 = require('./code-only.js');   // ★[T171] 주석 제거기 **정본**(acorn onComment · 사본 0). 옛 정규식 판은 `villages.js:20` 의 `// … sim/* …` 에 걸려 파일의 67.9% 를 삼켰다
