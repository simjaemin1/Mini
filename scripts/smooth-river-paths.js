...
//
// ★재민 지적: "눈여울… 그게 경계강이었던 거지? 두 강을 합친 거잖아? 근데 거기가 **미분불가능하게** 생겼어.
//   애초에 서쪽은 강을 잇는 점들이 빽빽한데 동쪽은 왜 느슨해? 이런 불연속 문제 해결하라고."
//
// 실측(수정 전 한반도):
//   눈여울 406점 — 최대 꺾임 **90.0°** @(26944,9696) · 점간격 서쪽 평균 74px vs 동쪽 **178px**(2.4배) · 최소 간격 0(중복점)
...
// ★균등 재샘플링만 하면 점이 3배로 불어난다(한여울강 460→1348, 한울대간 460→1473).
...
// 단순화만 하면 직선 구간이 통째로 한 구간이 되어 간격이 다시 들쭉날쭉해진다(한울대간 최대 6,238px).
...
// ★폭도 불연속일 수 있다 — 이음매 taper 가 남긴 계단(살여울천 228→127, 1.8배)이 실제로 남았다.
