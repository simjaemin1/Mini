...
function reduce(file) {
  ...
  if (!src.length) {
    const LOG = `/tmp/t371-${j.ARM && j.ARM !== 'off' ? W + '-' + j.ARM : W}.log`;
  ...
  return { W, ARM: j.ARM || 'off', file, ticks, pop, mob, clockNs, cutNs, cutUs, p50: p50.sort((a, b) => a - b)[Math.floor(p50.length / 2)],
...
for (const r of out) {
  ...
  console.log(`| **합** | **${r.sum.toFixed(2)}** | **${(r.sum / DENOM * 100).toFixed(1)}** | **${(r.sum / r.tot * 100).toFixed(1)}** | **${(r.sum / r.rest * 100).toFixed(1)}** | ${r.rows.length} |`);
