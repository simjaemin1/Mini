...
(() => {
  ...
  say('\n⑤ 실시간 환산 — 하루 24분 기준');
  {
    ...
    // ★판정이 아니라 **보고**다 — 재민 확정("첫 겨울 = 실시간 2~3주차")과 어긋나면 회부.
    const weeks = hrs / 24 / 7;
    say(`     ⇒ 재민 확정 목표는 "실시간 2~3주차"인데 현행은 **${weeks.toFixed(2)}주차**다`
      + `${(weeks < 2 || weeks > 3) ? '  ★어긋난다 — 조정은 회부(계절 주기)' : '  · 목표 안'}`);
  }
