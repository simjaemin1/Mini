...
function boot(name, file, env) {
  ...
  p.stdout.on('data', (b) => { const s = String(b); if (/up on|시딩|마을 시뮬 준비|사건 장부|사건 [0-9]/.test(s)) process.stdout.write(`  [${name}] ${s.trim().slice(0, 110)}\n`); });
  p.stderr.on('data', () => {});
  procs.push(p);
  return p;
}
...
(async () => {
  ...
  console.log('\n④-c [T80] 행 납품 — 버튼이 그 줄의 품목을 싣는가');
  {
    ...
    // ★★[T349 2026-09-21] 20회 예산 → **둘이 걸릴 때까지**(상한은 표에만).
    //   이 카드가 기동 게이트를 정본으로 옮기자 이 줄이 빨개졌다(`["herb 1/1","wood 2/2"]` → `["stone 3/3"]`).
