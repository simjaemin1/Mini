...
async function enter(browser) {
  ...
  await page.addInitScript(() => {
    window.__t380Spy = { req: 0, trees: 0, done: 0 };
    ...
    const _wire = (ws) => {
      ...
      OA.call(ws, 'message', (ev) => {
        ...
        else if (d.lastIndexOf('{"type":"far_trees_done"', 0) === 0) window.__t380Spy.done++;
      });
    };
...
(async () => {
  ...
  let browser = await chromium.launch({ headless: !HEADED });
  {
    ...
    ok(!!dbg && dbg.on === false && dbg.drawn === 0, `ⓐ4 층이 **한 그루도 안 그린다** (on=${dbg && dbg.on} · drawn=${dbg && dbg.drawn})`);
    await page.close();
  }
  await browser.close();
  ...
  await page.evaluate(() => window.__sendPrimary({ type: 'far_trees_req', r: 6000 }));
  const gotFar = await page.waitForFunction(() => window.__t380Spy.done >= 2, { timeout: 90000 }).then(() => true).catch(() => false);
  ok(gotFar, 'ⓑ4 큰지도 반경(6,000px) — 조각이 **끝까지** 왔다(far_trees_done 둘째)');
  await sleep(1000);
  const spy = await page.evaluate(() => window.__t380Spy);
  ...
  ok(offDbg.drawn === 0, `ⓑ9 대조군 — 끄면 **한 그루도 안 그린다** (drawn ${offDbg.drawn})`);
  ...
  const drawDelta = () => page.evaluate(() => new Promise((res) => {
    const n0 = window.__natDbg.treeDraw.n;
    requestAnimationFrame(() => requestAnimationFrame(() => res(window.__natDbg.treeDraw.n - n0)));
  }));
  ...
  ok(dOn - dOff >= backDbg.drawn * 0.5 && dOn > dOff,
     `ⓑ10 ★이 층이 **실제로 스프라이트를 돌린다** — 나무 그리기 호출이 프레임당 ${dOff} → ${dOn}`
     + ` (두 프레임 · 늘어난 ${dOn - dOff} ≒ 원경이 그린 ${backDbg.drawn} × 2)`);
  ...
  await page.evaluate(() => window.__sendPrimary({ type: 'far_trees_req', r: 9000 }));
  await sleep(6000);
