...
const HMAX = +(process.env.HMAX || 9), LAM = +(process.env.LAM || 10);
...
const hOf = (dE) => HMAX * (1 - Math.exp(-dE / LAM));
...
console.log('\n[큰 덩어리 10개]');
console.log('  ' + ['셀수', '가로', '세로', 'dmax', '봉우리m'].map(s => s.padStart(9)).join(''));
for (const r of rows.slice(0, 10))
  console.log('  ' + [r.n, r.w, r.h, r.dmax.toFixed(1), r.peak.toFixed(1)].map(s => String(s).padStart(9)).join(''));

console.log('\n[대안 — 발자국은 그대로, HMAX/LAM 만 바꾸면]');
console.log('  ' + ['HMAX', 'LAM', '중앙봉우리m', '90%봉우리m', '가장자리m/셀', '나무대비'].map(s => s.padStart(12)).join(''));
for (const p of [[9, 10], [20, 6], [35, 4], [35, 2.5], [60, 3], [60, 1.8]]) {
  const f = (dE) => p[0] * (1 - Math.exp(-dE / p[1]));
