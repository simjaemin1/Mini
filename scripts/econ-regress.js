...
const fs = require('fs');
// ★★★[T73 2026-09-03 · 계측기 결함 수리] **이 줄이 `/root/minirepo` 로 못 박혀 있었다.**
//   그 경로는 이 저장소가 **아니다** — 컨테이너에 남아 있던 **다른 체크아웃**(3ee3569 · 낡음)이다.
//   그래서 이 회귀는 여태 **작업 트리가 아니라 그 낡은 트리**를 재고 있었고, 그러니 무엇을 고쳐도
//   숫자가 안 움직였다("불변"이 아니라 **안 본 것**이다). 같은 파일 머리 주석이 경고하는 바로 그 species:
//     *"전부 '랩이 본 게임과 다른 것을 재고 있었다' 였다."*
//   ⇒ 실측으로 잡았다: `T73_RAWGRAIN` 을 켜고 꺼도 5시드 표가 **한 자리도 안 움직였는데**,
//     같은 인자로 CLI 를 직접 돌리면 시드 42 인구가 **513 → 517**, 생곡 재고가 17,147 → 13,161 로 움직인다.
//   ⚠**옛 보고의 `econ-regress` 줄은 이 결함 위에서 쓰였다**(T17·T60 의 "2,616 · 651 · 13,473 불변").
//     그 줄들은 **재측정 대상**이다 — `보고/T73_*.md` §2 에 옛/새를 다시 적었다.
