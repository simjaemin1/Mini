#!/usr/bin/env node
// === scripts/t274-board-ab.js — T274: 게시판 품목 집계 + 좁은 팔("게시판만 식사를 본다") ==========
//
...
//   ARM=base|board       base = 전부 끔 · board = `T274_BOARD_FOOD=1`(게시판 둘만 식사를 본다)
//        |food|both       (T263 팔도 그대로 남긴다 — 나란히 놓고 읽으려고)
// ★이 계측기가 T263 에 더한 것: **게시판 품목 집계**(`onEvent`·`onRequest` 로 품목별 건수를 센다 — 유도가 아니라 집계).
//   AB_SAMPLE=10  AB_JSON=/tmp/x.json
'use strict';
//   팔 = [식사 훅(T263) · 부재료 훅(T258) · 게시판 좁은 팔(T274)]
const _ARMS = { base: [0, 0, 0], food: [1, 0, 0], both: [1, 1, 0], board: [0, 0, 1] };
const ARM = _ARMS[process.env.ARM] ? process.env.ARM : 'base';
{
  const [f, si, bd] = _ARMS[ARM];
  ...
  if (bd) process.env.T274_BOARD_FOOD = '1'; else delete process.env.T274_BOARD_FOOD;
}
...
const BoardFood = R('server/board-food-ema');
const KF = CK.foodConsOn(), KS = CK.sideConsOn(), KB = BoardFood.boardFoodOn(), _W = _ARMS[ARM];
if (KF !== !!_W[0] || KS !== !!_W[1] || KB !== !!_W[2]) { console.error(`손잡이 상태가 팔과 안 맞는다(ARM=${ARM} · food=${KF} · side=${KS} · board=${KB})`); process.exit(2); }
...
console.log(`  손잡이    T263_FOOD_CONS=${KF ? '1' : '미설정'} · T258_SIDE_CONS=${KS ? '1' : '미설정'} · T274_BOARD_FOOD=${KB ? '1' : '미설정'}`
...
if (process.env.AB_JSON) {
  ...
  fs.writeFileSync(process.env.AB_JSON, JSON.stringify({ arm: ARM, knobFood: KF, knobSide: KS, knobBoard: KB, tally: TALLY, seed: SEED, days: DAYS, track: TRACK, sides: SIDES, nLive: nLive2,
