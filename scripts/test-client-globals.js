...
// ★[T235 2026-09-13] `30-n-net.js` 13 → **14**. `window.__centralCall(door, extra)` 하나가 늘었다:
...
const BASELINE = {
  ...
  // ★[T155 2026-09-07] `41-h-bubble.js` 17 → **18**. 시험 손잡이 하나가 늘었다:
  //   `window.__setCharMocap2(v)` — 모션 둘째 판(CMU 모캡 클립 셋) 대조용 토글.
  //   바로 위의 `__setCharSprite` 와 **같은 자리·같은 문법**이고 그것이 이미 17 안에 들어 있다.
  //   서버 env 정본은 아직 없다(T155 는 서버 무접촉) — 켜기는 재민 눈 판정 뒤 카드 몫.
  'client/41-h-bubble.js': 18, 'client/44-h-hud.js': 1,
