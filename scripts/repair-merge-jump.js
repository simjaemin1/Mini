...
//   죽림천은 이음매가 path[0](머리)이었는데 선천을 꼬리에 붙여 버려서,
//   (20224,35776) → (21377,21314) 로 **14,508px(453셀)** 을 폭 475px로 가로지르는 직선이 생겼다.
//   명호(21984,23136) 바로 옆을 지나간다. 정확히 그 강이다.
//
//   덤으로 이음매 인덱스도 어긋나서, 폭 taper 가 진짜 이음매가 아니라 죽림천 **반대쪽 끝**에 걸렸다.
//
// 수리: 죽림천 구간을 원본(6581c36)에서 되살려 **뒤집어** 붙이고, 이음매를 제자리에서 다시 taper 한다.
...
const A0 = orig.rivers.find((r) => r.name === '죽림천' && !r._mirroredFrom);
const rv = d.rivers.find((r) => r.name === '선천' && !r._mirroredFrom);
...
const B = rv.path.slice(j + 1);                       // 선천 구간(현행 — 율호 연장 포함)
const A = A0.path.slice().reverse().map((q) => ({ pos: [R(P(q)[0]), R(P(q)[1])], width: R(q.width != null ? q.width : 200) }));
console.log('  죽림천 원본 ' + A0.path.length + '점을 뒤집어 앞에, 선천 구간 ' + B.length + '점을 뒤에');
