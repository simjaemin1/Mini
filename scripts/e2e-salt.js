...
(async () => {
  ...
  const _zone = boot('zone', 'zone.js', { PORT: String(ZPORT), ZONE_ID: 'hanbando', DB_PATH: ZDB, CENTRAL_URL: `http://localhost:${CPORT}`,
  ...
  // ★★[T160 2026-09-07] **회부 D-1 이 증상을 잘못 적고 있었다 — 재서 고쳤다.**
  //   D-1 은 "비용 칸에 영문 키 `brine` 이 뜬다"였는데, 패널 글자에는 `brine` 도 `짠물` 도
  //   **둘 다 없었다**. 키는 `title` 속성에만 살아 `textContent` 엔 안 잡힌다 —
  //   즉 화면은 **그림과 숫자만** 보여 주고 이름은 아예 없었다(`itemIconHtml` 의 셋째 인자가
  //   T66 이후 무시된다 · `43-i-icon.js:105`). 이 하네스도 그 이분법을 그대로 믿어
  //   "= 영문 키 brine" 이라고 **자기가 못 본 것을 찍고 있었다**. 셋을 갈라 적는다.
  const brineRaw = /brine/.test(kt), brineKo = /짠물/.test(kt);
  ok(brineKo, '★★③ 재료 이름이 **한글로** 뜬다 — 서버 `costKo` 정본이 화면까지 닿는다',
     brineKo ? '짠물' : (brineRaw ? '영문 키 brine' : '이름이 아예 없다(그림+숫자만)'));
  ok(!brineRaw, '★③ 그리고 영문 키는 안 뜬다', brineRaw ? 'brine 이 글자로 보인다' : '없다');
  console.log(`  · [실측] 짠물 표시 = ${brineKo ? '한글' : brineRaw ? '영문 키 brine' : '이름 없음'}`
              + ` · 보관 칸 = ${/보관 ∞/.test(kt) ? '∞(뜻대로)' : '수치'}`);
