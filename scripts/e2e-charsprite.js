...
(async () => {
  ...
  //   시트 행의 '향'은 **그 행 실루엣의 폭**으로 읽는다. 이건 메타의 주장이 아니라 **시트 그림 자체**다.
  //   ★★[T96 2026-09-05] 폭을 만드는 것이 무엇인지가 바뀌었다 — 아래 판정 주석에 실측이 있다.
  //     종전 근거는 **어깨**였다(*"옆에서 보면 좁고 앞뒤로 보면 넓다"*). 그런데 재 보니 폭을 정하는 건
  //     어깨가 아니라 **보폭**이다: 한 걸음은 1m 가까이 벌어지고 어깨는 0.4m 다. 걸음이 앞뒤(시상면)로
  //     제대로 나가는 지금, 옆모습(=화면 가로 이동)이 **가장 넓고** 앞뒤 모습이 가장 좁다.
  //     옛 시트에선 걸음이 좌우로 벌어져 있어 그 관계가 정확히 90° 어긋나 있었다.
  console.log('\n=== ⓓ 8방향 — 걷는 화면 방향 ↔ 시트 행 [T57] ===');
  {
    ...
    // ★★[T96 2026-09-05] **이 판정의 부호가 뒤집혔다 — 종전 규칙은 결함을 규격으로 적고 있었다.**
    //
    //   종전: *"화면 가로로 걸으면 가장 **좁은** 축, 세로면 가장 **넓은** 축"*.
    //   T96 이 리그를 `depsgraph` 로 재 보니 옛 사인 걸음은 다리를 캐릭터의 **좌우(rx)** 로만 흔들었다
    //   — 걷는 게 아니라 **가랑이를 옆으로 벌린 것**이었다(넓적다리 월드 방향의 앞뒤 성분이 정확히 0).
    //   그래서 폭이 **바라보는 쪽과 직각**으로 커졌고, 위 규칙은 그 직각 관계를 적은 것이다.
    //
    //   ★모캡으로 갈아 끼운 지금은 보폭이 **시상면(바라보는 쪽)** 에 있다. 그러면 기하가 이렇게 말한다:
    //     화면 가로로 걷는 방향의 월드 벡터는 화면에 **(±90.5, 0)** 로 투영된다(가장 길고 순수 가로) —
    //     다리가 화면 가로로 벌어지니 실루엣이 **가장 넓다**.
    //     화면 세로로 걷는 방향은 **(0, ±45.3)** (가장 짧고 순수 세로) — 가로 폭이 **가장 좁다**.
    //   ★말이 아니라 **실측이 증명한다**(같은 `body_walk` 행별 실루엣 폭):
    //     옛(사인)  [33, 42, 33, 12, 33, 42, 33, 12]     ← 최대 행 1·5 · 최소 행 3·7
    //     새(모캡)  [35, 29, 36, 40, 35, 29, 36, 40]     ← 최대 행 3·7 · 최소 행 1·5
    //     최대와 최소가 **정확히 두 행(90°) 옮겨 갔다.** 좌우 벌림 ↔ 앞뒤 보폭의 차이 그대로다.
    //     (옛 행 3 의 폭 12 는 두 다리가 시선축에 정확히 겹쳐 **막대기**가 된 것이다.)
    //   ⇒ 규칙을 뒤집는다. **문턱과 여유(0.34/0.15)는 안 건드린다** — 느슨해진 게 아니라 방향만 바꿨다.
    const wMin = Math.min(...sheetW), wMax = Math.max(...sheetW);
    let good = 0, bad = [];
    for (const r of rows) {
      if (r.w == null) { bad.push(`${r.arrow}:행없음`); continue; }
      const okRow = r.axis === '가로' ? (r.w >= wMax - (wMax - wMin) * 0.34)
                  : r.axis === '세로' ? (r.w <= wMin + (wMax - wMin) * 0.34)
