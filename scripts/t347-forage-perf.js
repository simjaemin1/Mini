#!/usr/bin/env node
// === scripts/t347-forage-perf.js — 세계 위 자: 나무꾼 행위의 값 (T347 ③ · T284·T316 문법) ======
//
...
//     off  `T347_FORAGE_ACT` 미설정   — 종전 수식
//     on   `T347_FORAGE_ACT=1`        — 나무꾼이 걷고 베고 나른다
//
// ★내는 수: 존 틱 본문 p50/p95/max · 틱 빚(dropN · lagPct) · 벌목하는 사람 수 · 손에 든 사람·kg ·
//           **첫날 목재 합 ↔ 수식**(`delivered` ↔ `formulaPerDay`) · 나무 없는 마을 수
//
// 실행: node scripts/t347-forage-perf.js [out.json]
...
const OUT = process.argv[2] || '/tmp/t347/forage-perf.json';
...
(async () => {
  ...
  if (!fs.existsSync(TPL) || process.env.REWARM === '1') {
    rmdb('/tmp/t347-z-warm.db');
    say(`틀 세계 굽기 — ${WARM_DAYS}일`);
    const b = boot('warm', { DB_PATH: '/tmp/t347-z-warm.db', VILLAGE_DAY_MS: String(Math.max(1500, Math.floor(DAY_MS / 10))) });
    ...
    rmdb(TPL);
    for (const s of ['', '-wal', '-shm']) { try { fs.copyFileSync('/tmp/t347-z-warm.db' + s, TPL + s); } catch (e) {} }
    res.warmDay = d; say('틀 완료 day', d);
  }
  for (const arm of ARMS) {
    const db = `/tmp/t347-z-${arm}.db`; rmdb(db);
    for (const s of ['', '-wal', '-shm']) { try { fs.copyFileSync(TPL + s, db + s); } catch (e) {} }
    const env = { DB_PATH: db, VILLAGE_WAR_LOG: '0' };
    if (arm === 'on') env.T347_FORAGE_ACT = '1';
    say(`팔 ${arm} — T347_FORAGE_ACT ${arm === 'on' ? '1' : '(미설정)'}`);
    ...
    const first = [];
    for (let i = 0; i < 120 && d < d0 + 2; i++) { await sleep(2000); const p = await b.perf(false); d = dayOf(p); if (p && p.forage) first.push({ d, delivered: p.forage.delivered, formulaPerDay: p.forage.formulaPerDay, cells: p.forage.cells, act: p.forage.actVillages }); }
    await b.perf(true);
    const samples = [];
    const t0 = Date.now();
    while (Date.now() - t0 < WINDOW_S * 1000) { await sleep(20000); const p = await b.perf(false); if (p) samples.push({ t: Math.round((Date.now() - t0) / 1000), day: dayOf(p), walkers: p.forage ? p.forage.walkers : null, hands: p.forage ? p.forage.hands : null, delivered: p.forage ? p.forage.delivered : null }); }
    ...
    res.arms[arm] = { day: dayOf(p),
      ...
      loop: p && p.loop, wood: p && p.forage, first, samples };
    ...
    say(`  ${arm}: 틱 p50 ${t ? t.p50 : '?'} p95 ${t ? t.p95 : '?'} max ${t ? t.max : '?'} · drop ${p && p.tick ? p.tick.dropN : '?'} · lag ${p && p.tick ? p.tick.lagPct : '?'}%`
      + (p && p.forage ? ` · 걷는 채집꾼 ${p.forage.walkers} · 손 ${p.forage.hands}(${p.forage.handU}kg) · 입고 ${p.forage.delivered} · 수식/일 ${p.forage.formulaPerDay}`
        + ` · 행위 ${p.forage.actVillages} · 군락0 ${p.forage.noGroveVillages} · 군락 ${p.forage.groves}(K ${p.forage.K}) · 되살 ${p.forage.back} · 한도 ${p.forage.cap} · 딴 ${p.forage.pickDay}`
        + ` · 인구 ${p.forage.popAll} · 사람당 ${p.forage.popAll > 0 && t ? (t.p50 * 1000 / p.forage.popAll).toFixed(3) : '?'}µs` : ''));
