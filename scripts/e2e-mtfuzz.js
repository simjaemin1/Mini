...
(async () => {
  const _central = boot(path.join(ROOT, 'server', 'central.js'), { PORT: '' + CPORT, PUBLIC_HOST: 'localhost', ENABLED_ZONES: 'hanbando' });
  ...
  const _zone = boot('/tmp/zw-fz.js', { PORT: '' + ZPORT, ZONE_ID: 'hanbando', DB_PATH: '/tmp/fz.db', CENTRAL_URL: `http://localhost:${CPORT}`,
  ...
  if (!await (await _zp).ok) { console.log('zone 기동 실패'); process.exit(1); }
  await sleep(4000);
