...
# ★★[T145 2026-09-06] **되살렸다 — 이번엔 굽고 배포한다.**
#   T120 은 이 반복 40종을 지웠다 — 굽는 표에만 있고 배포본이 0 이어서다("굽지 않는 재료표는
#   읽는 사람을 속인다"). 그 카드가 왔고, 지울 때 적어 둔 그 자리에서 그대로 가져왔다
#   (`git show 0175f2b5^:scripts/bake-mountain.py`). **새 형상 0** — 숫자 한 자 안 바꿨다.
#   ⚠이제 이 표는 45+40+3 = 88 이고 배포도 88이다. 둘이 갈리면 다시 거짓말이 된다.
for a in range(8):
