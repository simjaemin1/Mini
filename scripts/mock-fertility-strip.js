...
function go(){
  ...
  g.fillText('산이었던 타일 (비옥도 램프 + 영구 지질 표식: 박힌 바위·노두)', 10, (H+40)*2+18);
  [0,1].forEach(row=>{
  stages.forEach((f,i)=>{
    ...
    if(row===1){
      // ★영구 지질 표식 [재민 "산이었던 타일은 구분을 줘야 해"] — 비옥도와 무관하게 남는 박힌 바위·노두
      for(let k2=0;k2<4;k2++){
        const im=imgs.rock; const sc=(26+26*hash(9+i,k2,71))/im.height;
        const bx=x0+18+hash(9+i,k2,72)*(W-56), by=y0+46+hash(9+i,k2,73)*(H-78);
        g.globalAlpha=0.55; g.fillStyle='#4a453c';
        g.beginPath(); g.ellipse(bx+im.width*sc/2, by+im.height*sc-3, im.width*sc*0.62, 5, 0, 0, 6.3); g.fill();  // 뿌리 그늘(박힘)
        g.globalAlpha=1;
        g.drawImage(im,bx,by,im.width*sc,im.height*sc);
      }
    }
    g.restore();
    g.fillStyle='#ddd'; g.font='bold 15px sans-serif'; g.textAlign='center';
    g.fillText('비옥도 '+f, x0+W/2, y0+22);
    const label=f===0?'암반(채굴 직후)':f===150?'자갈밭':f===300?'맨흙':f===450?'마른 땅':f===600?'회복 중':f===800?'★산 파괴 직후':f===900?'풀 돋음':'초원(완전 회복)';
