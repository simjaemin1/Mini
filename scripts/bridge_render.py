...
M['ditchw'] = bumped_mat("ditchw", (0.30, 0.23, 0.15), (0.19, 0.14, 0.09), 14, 0.9, 0.96)   # ★11차 T3 도랑 벽면(파낸 생흙 — 지면보다 어둡고 거칠게)
M['ditchb'] = bumped_mat("ditchb", (0.085, 0.062, 0.042), (0.045, 0.032, 0.022), 10, 0.5, 0.99)   # ★도랑 바닥 — 아주 어둡게. 아이소에선 깊이를 기하로 못 보여 주므로 **밝은 테두리 + 어두운 속**이 유일한 '구멍' 신호(3패스 실측)
M['spoil']  = bumped_mat("spoil",  (0.40, 0.31, 0.20), (0.28, 0.21, 0.13), 13, 0.7, 0.95)   # 파낸 흙더미(둔덕 — 도랑 안쪽 가장자리)
...
# ===== 환호(도랑) 타일 — 11차 T3. 1셀 규약(크롭 금지·이미지 중심=셀 중심·128px) =====
#   고증(우리역사넷 「전쟁의 기원-마을의 출현」): 검단리 환호 **깊이 20~110cm · 폭 가장 넓은 곳 2m**.
#   1셀=1m 실축이므로 **한 셀 = 도랑 폭의 절반**이다. 그래서 타일은 "도랑 한복판의 1m 구간"이고,
#   양옆 셀과 이어져야 폭 2m 도랑으로 읽힌다 → **횡단면 벽을 셀 경계에 만들지 않는다**(타일러블 규약).
#   즉 종방향(도랑이 흐르는 방향)으로는 x=0..1 전 구간 관통, 횡방향으로만 벽을 세운다.
# ★1·2패스 실측 교정 — 아이소에서 **깊은 도랑은 담벼락으로 읽힌다.**
#   30° 고도각이라 깊이 d의 먼 벽 그림자가 d/tan30° = 1.73d 만큼 바닥을 덮는다.
#   d=0.55 → 0.95m(셀 폭 전부 가림) · d=0.35 → 0.61m. 둘 다 바닥이 거의 안 보여 '세워 놓은 판'이 됐다(실측).
#   ⇒ 기하로 깊이를 표현하지 않는다. **얕은 홈(0.13m) + 어두운 바닥 + 바깥쪽 파낸 흙 둔덕**으로
#     "판 자리"를 색·둔덕으로 읽힌다(아이소 관례). 고증 깊이(20~110cm)는 게임 판정(=이동 불가)이 표현한다.
DITCH_DEPTH = 0.13        # 렌더용 홈 깊이(판독성 실측 확정) — 고증 깊이는 콜라이더가 표현한다
def _ditch_run(corner=False):
    """도랑 한 칸: 어두운 파인 바닥 + 얕은 턱 + 바깥쪽 파낸 흙 둔덕. 종방향 관통(타일러블)."""
    random.seed(41)
    # ★실화면 실측 교정: 바닥을 0.56폭으로 두니 셀의 31%만 덮여 **띄엄띄엄한 점무늬**로 보였다(링이 안 이어짐).
    #   다리 널(PLANK_SPAN=1.06)과 같은 규약으로 **셀보다 살짝 크게** 덮어 이웃 타일과 알파 가장자리가 겹치게 한다.
    box(1.04, 1.04, 0.02, (0.5, 0.5, -DITCH_DEPTH), mat=M['ditchb'])            # 파인 바닥(어둡게·셀 전체)
    if not corner:
        for wy in (0.0, 1.0):                                                    # 횡방향 턱만(종방향은 뚫려 이웃과 이어짐)
            box(1.06, 0.16, DITCH_DEPTH, (0.5, wy, -DITCH_DEPTH / 2), mat=M['spoil'])   # ★턱 = 밝은 생흙(파낸 흙이 가장자리에 눌린 자리) — 어두운 속과 대비를 만든다
        for i, t in enumerate((0.24, 0.62)):                                     # 파낸 흙 둔덕 — 바깥 한 줄만 띄엄띄엄(사다리처럼 안 보이게)
            ico(random.uniform(0.070, 0.092), (t, 1.16, 0.02), subdiv=1, mat=M['spoil'], jitter=0.35, scale=(1.9, 0.9, 0.42))
    else:
        for (wx, wy, sx, sy) in ((0.0, 0.5, 0.16, 1.06), (1.0, 0.5, 0.16, 1.06), (0.5, 0.0, 1.06, 0.16), (0.5, 1.0, 1.06, 0.16)):
            box(sx, sy, DITCH_DEPTH * 0.6, (wx, wy, -DITCH_DEPTH * 0.7), mat=M['spoil'])   # 사방 얕은 턱(꺾임 이음매 없음)
        for (mx, my) in ((0.14, 1.08), (0.5, 1.10), (1.08, 0.5)):
            ico(0.080, (mx, my, 0.02), subdiv=1, mat=M['spoil'], jitter=0.35, scale=(1.5, 1.2, 0.5))


def t_ditch_x():   # 가로(월드 x축) 도랑 — y=0/1 쪽에 벽, x축으로 관통
    _ditch_run(False)


def t_ditch_c():   # 모서리·교차 — 사방 열린 웅덩이(링 꺾임에서 이음매 없음)
    _ditch_run(True)


JOBS = [("bridge_mid", t_mid, True), ("bridge_cap1", t_cap1, True), ("bridge_cap0", t_cap0, True),
        ("ditch", t_ditch_x, True), ("ditch_c", t_ditch_c, False),
