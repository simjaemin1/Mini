...
if (!DO_LAB) {
  ...
  const run = (seed, flipDay) => {
    ...
    // ★"왜 안 변했나"를 하네스가 스스로 답하게 한다 — NPC 가 닿는 광맥에 철이 있는가.
    //   1744e0e 이후 **주요 광맥의 철은 0**이다(플레이어 전용 자잘에 184개). 그래서 시대를 열어도
    //   NPC 경제엔 녹일 철이 없다 — 변화 0% 는 배선 고장이 아니라 **설계 그대로**다.
    agg.feVillages = d.villages.filter((v) => ((v.oreMix || {}).iron || 0) > 0).length;
    return agg;
  };
  const rows = [];
  ...
  if (rows.length) {
    ...
    // ★변화가 0 이면 그 이유를 하네스가 말해 준다 — "안 변했다"와 "배선이 죽었다"는 다르다.
    const feV = mean('feVillages', 'a');
    if (feV < 0.5) {
      say('');
      say('   ★변화 0% 의 이유: **NPC 가 닿는 광맥에 철이 한 톨도 없다**(철 광종 마을 0곳).');
      say('     주요 광맥 철 0 = 설계 그대로다(1744e0e — 철은 플레이어 전용 자잘 광맥 184개에만 있다).');
      say('     ⇒ 시대를 여는 것은 지금 **NPC 경제에 무영향**이고, 바뀌는 건 오직 플레이어 축이다:');
      say('       괴련로 건설 해금 · 같은 도가니로의 철 수율 3.4% → 67.8% · 말 길들이기.');
      say('     ⇒ NPC 철기 산업을 원한다면 시대만으로는 안 되고 **주요 광맥에 철을 넣어야** 한다(회부 대상).');
    } else {
      say(`   (철 광종 마을 ${feV.toFixed(1)}곳 — NPC 도 철을 캘 수 있는 세계다)`);
    }
  }
}
