...
## X-8. ★2026-09-26 — T413 `test-war-world` ⓛ 간헐 빨강 = 자 · ⑥ 선포 무관 동원 표

> 카드 `지시/지시_T413.md` · 보고 `보고/T413_2026-09-26.md` · 가지 `batch/war-flaky-0926` · 운영 코드 0줄.

### 뿌리 둘(둘 다 하네스)

1. **시계가 둘** — 판은 가상 날(`world.day`)로 도는데 운영 `spawnOneNpc`(`villages:762`)는 태어날 자리를 `gameDayOf(Date.now())` 로 씨 뿌린다. 하네스 state 는 `epoch 0 · dayMs 30000` 이라 그 값이 **벽시계 30초마다** 바뀐다 ⇒ 같은 씨인데 돌린 시각에 따라 다른 판.
2. **전멸 판의 끝** — 공격 표본이 전멸하면 `_warEndFight` 가 `ended` 를 적고 같은 틱에 몸을 치운다(`:3483`·`:3486`). 옛 하네스는 살아 있는 몸에서만 `ended` 를 읽어 그 판을 "안 끝났다"로 셌다. 스폰 날 22개 중 5개가 그 판(≈23%)이다.

### 고침(하네스만)

* 스폰 순간만 `Date.now` 를 판의 날(`opts.spawnDay ?? world.day`)에 맞춘다 — `try … finally` 한 자리.
* 끝은 몸이 남긴 기록으로 읽는다(`tr._body` 참조 · 판 밖으로는 안 내보낸다) · 옛 규칙은 `tr.endedLive`(미끼).
* 절 ⓤ 6칸 — 벽시계를 밀어도 같은 판 · 전멸 판(날 406)도 끝 · 미끼 셋.

### ⑥ 선포 무관 동원 — 지금

* 선포 = `warMobilize` 의 **로그 한 줄**(`war-core:678`) · 동원·행군과 같은 틱 · 상대는 몸을 볼 때(경보 거리)만 안다 ⇒ **NPC 전쟁은 전부 기습**.
* 설계 한 줄: **선포 = 사자가 걸어간다**(T7 소문 문법) — 도착해야 상대가 안다. ⚠소문 시계(520셀 ≈ 3일)가 군대(1일)보다 느리다 → 사자는 행군 시계가 PM 초안(재민 판정).
