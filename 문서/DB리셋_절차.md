...
## 5-0. ~~라이브 리셋 (fly 판)~~ ★★플랫폼 오류 — 쓰지 마라 (2026-08-03 재민 검증 세션)

> **이 절은 라이브가 fly 에 있다고 잘못 가정하고 쓰였다.** 실제 라이브는 **Vultr 도커**다
> (`root@141.164.35.114` · 존 컨테이너 `durango-zone-hanbando` · DB `/srv/durango/hanbando/`).
> fly.toml 은 초기 계획의 잔재이고 fly 에는 아무것도 안 떠 있다.
> **정본은 `라이브반영_런북_v3_50마을.md`(v3 — 50마을 전환판. 옛 파일명 라이브반영_런북_2026-08-03.md 에서 개명)** — 리셋+배포를 한 흐름으로 담고 있다.
> 아래 fly 블록은 오류의 기록으로만 남긴다(§5 로컬/도커 판이 맞는 절차였다).

## 5-0-구판. (오류 — fly 가정) 라이브 리셋 — 복사·붙여넣기 한 블록 (2026-08-02e)
